...
你是 session B，角色是 even-number writer。
...
3. 只有在下一個數字是偶數時，你才能動作
4. 你每次只能追加一個數字到 `result.txt`
5. 不可修改既有內容，不可一次追加多個數字
6. 追加後更新 `HANDOFF_LOG.md`
7. 若使用 git，commit message 格式必須是：`[role:b] append <數字>`
8. 若下一個數字不是你負責的，則不要寫入，直接停止並回報目前輪到 A
9. 若最後一個數字已是 `100`，直接停止並回報完成

限制：

- 不可寫入奇數
- 若 `result.txt` 不存在，不可自行初始化
- 不可跳號
- 不可一次做多輪
