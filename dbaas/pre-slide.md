...
## 10. 資安與治理現況
- `AppProject` 已限制可部署 namespace，具備最小治理骨架
- 目前 `percona-operator` RBAC 偏寬鬆，不適合直接沿用到正式環境
- `mysql-single` 密碼仍放在 GitOps secret manifest，正式環境需改為安全憑證管理
...
- 收斂 TiDB scheduler 與正式版相容性策略
- 收斂正式環境 RBAC、Secret 管理與對外入口策略
- 若要進入真正 DBaaS，下一階段需補自助申請入口、審批流程、配額與標準規格模型
- 之後才適合擴展到 Backup / Restore / Upgrade / Decommission 的標準化工作流

## 建議簡報呈現方式
- 第 1 張：用一張圖講清楚 GitOps、Argo CD、Operator、DB workloads 的關係
- 第 2 張：列出目前已完成驗證的服務矩陣（MySQL / Redis / TiDB / Monitoring）
- 第 3 張：列出 `已完成 / 未完成 / 風險` 三欄，避免聽眾誤以為已是正式平台
- 第 4 張：用 `mysql-single-1..10` 展示多產品接入與 namespace 隔離概念
- 第 5 張：收斂成正式環境導入前的待補清單
...
#### 主要風險
- 現行儲存為 `local-path`，只適合 Lab，不適合正式高可用場景
- 部分權限與 Secret 管理方式仍偏 POC，若直接上線有治理風險
- TiDB 與部分 Operator 組態仍帶有版本相容性限制
- 目前對外連線模式偏工程實驗用途，尚未收斂成企業標準接入方案
- 若未先補齊治理與流程，平台容易變成「可部署 DB」而非「可治理的 DBaaS」

#### 結論句
- 目前已證明 DB workload 可透過 GitOps 標準化交付；下一階段重點不是再加更多 DB，而是補齊治理、資安、HA/DR 與自助化流程。
