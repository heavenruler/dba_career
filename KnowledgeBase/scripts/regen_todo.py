...
def build_script(docs: list[dict]) -> str:
    ...
    lines.append("  fi")
    lines.append("")
    lines.append("  # 5h window 額度保護：剩餘 < 10% 即停（保留緩衝避免硬封）")
    lines.append("  # 來源：filter_doc.py 每篇印 `5h window used=X% remaining=Y%`，抓最新一筆")
    lines.append("  REMAIN=$(grep -oE '5h window used=[0-9.]+% remaining=[0-9.]+' \"$LOG\" 2>/dev/null | tail -1 | sed -E 's/.*remaining=//')")
    lines.append('  if [[ -n "$REMAIN" ]] && awk -v r="$REMAIN" \'BEGIN{exit !(r+0 < 10)}\'; then')
    lines.append('    echo "[todo.sh] 5h window remaining=${REMAIN}% < 10% → 停止以保護額度" | tee -a "$LOG"')
    lines.append('    echo "  resume: make filter_all  (5h window 重置後再跑)" | tee -a "$LOG"')
    lines.append("    exit 0")
    lines.append("  fi")
    lines.append("done")
    lines.append("")
