...
def build_script(docs: list[dict]) -> str:
    ...
    lines.append("")
    lines.append("# 5h window 額度檢查 — 啟動時 & 每篇開跑前都檢")
    lines.append("# 邏輯：取 filter_progress.log 最新 5h 行；若 resets_at 已過 → 視為新 window 放行；")
    lines.append("#       否則 remaining < 10% 即 exit 0（保護額度），用戶可等 window 重置後再跑")
    lines.append("check_5h_quota() {")
    lines.append('  local line remain resets_at resets_epoch now_epoch')
    lines.append("  line=$(grep -oE '5h window used=[0-9.]+% remaining=[0-9.]+% resets@[0-9-]+ [0-9:]+' \"$LOG\" 2>/dev/null | tail -1)")
    lines.append('  [[ -z "$line" ]] && return 0    # 沒紀錄 → 放行')
    lines.append('  remain=$(echo "$line" | sed -nE \'s/.*remaining=([0-9.]+).*/\\1/p\')')
    lines.append('  resets_at=$(echo "$line" | sed -nE \'s/.*resets@(.+)$/\\1/p\')')
    lines.append('  resets_epoch=$(date -j -f \'%Y-%m-%d %H:%M\' "$resets_at" \'+%s\' 2>/dev/null || echo 0)')
    lines.append("  now_epoch=$(date '+%s')")
    lines.append('  if [[ "$resets_epoch" -gt 0 ]] && [[ "$now_epoch" -ge "$resets_epoch" ]]; then')
    lines.append('    # log 是上一個 window 的，window 已重置 → 新一輪，放行')
    lines.append("    return 0")
    lines.append("  fi")
    lines.append('  if awk -v r="$remain" \'BEGIN{exit !(r+0 < 10)}\'; then')
    lines.append('    echo "[todo.sh] 5h window remaining=${remain}% < 10% (resets@$resets_at) → 停止以保護額度" | tee -a "$LOG"')
    lines.append('    echo "  resume: make filter_all  (在 $resets_at 之後)" | tee -a "$LOG"')
    lines.append("    exit 0")
    lines.append("  fi")
    lines.append("}")
    lines.append("")
    ...
    lines.append("check_5h_quota   # 啟動就先檢一次，剩餘 <10% 直接退出")
    lines.append("")
    lines.append("DOCS=(")
    ...
    lines.append("  fi")
    lines.append("")
    lines.append("  check_5h_quota   # 每篇開跑前先檢，避免燒完 1 篇才發現額度爆")
