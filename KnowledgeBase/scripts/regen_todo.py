...
def build_script(docs: list[dict]) -> str:
    ...
    lines.append('    echo "[dry-run] make filter_doc DOC_ID=$DOC"')
    lines.append("    continue")
    lines.append("  fi")
    lines.append("")
    lines.append("  DONE=$((DONE + 1))")
    lines.append("  TS=$(date '+%H:%M:%S')")
    lines.append('  echo "[$TS] [$DONE/$TOTAL] filter $DOC" | tee -a "$LOG"')
    lines.append('  if make filter_doc DOC_ID="$DOC" >>"$LOG" 2>&1; then')
