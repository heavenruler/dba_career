...
| <a id="note-3"></a>註3 | **資料品質口徑**：本表所有 v4.7 結果均為 **5-round mean**（R1-R5 全部納入；canonical 詳 [PoC-DESIGN §8.3](./PoC-DESIGN.md#83-round-採樣策略) 與 [PHASES §5](./PHASES.md#5-summaryjson-schema-與-metadata)），由 [`tests/common/summary-from-stdout.py`](../tests/common/summary-from-stdout.py) 解析 stdout 後落地至各 suite `summary.json`；pre-v4.7 single-run 結果已封存於 `results/archive/` 底下的歷史 scope（例如 `archive/cockroach-tc1/S-BASE/archive/`、`archive/yuga-tc1-old/S-BASE/archive/`），不納入本表。 |
