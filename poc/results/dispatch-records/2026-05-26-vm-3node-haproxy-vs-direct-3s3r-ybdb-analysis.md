...
> direct 5-round mean = 8,729 反映含 outlier round（min=4,409），數據 raw 範圍 4,409–9,852 — 即 4-cell analysis 報告 §5 的「3s3r 極不穩」現象。haproxy 對應 range 15,018–16,122 — 一條線跑出來。
...
### 為什麼 PoC-DESIGN §6.4「YBDB HAProxy delta 預期最小」錯

原假設：「YBDB tserver 一體（內部 leader-aware routing），client 入口分散沒幫助」  
實測證明：**4 vCPU + 3 tservers + RF=3 + 3 shards 下，single entry point 的 YSQL postgres 是隱藏吞吐瓶頸**，與 tserver 內部 routing 機制無關。

PoC-DESIGN §6.4 第 222 行原文：「TiDB 預期最大（SQL 層 stateless 可水平分散），YBDB 預期最小（tserver 一體）」。本次數據要求修訂為：YBDB HAProxy delta 在本硬體規格 **+79%**（best mean 對比）。
...
3. **PoC-DESIGN §6.4 跨家 HAProxy delta 排序假設失效**：原假設「TiDB 最大 → CRDB → YBDB 最小」，本實測證明 YBDB 在 RF=3 sharded 拓樸下 **+79%** — 不是「最小」。需待 tidb / crdb haproxy 變體跑完才能確定跨家排序
