...
| 3 | ybdb | 3s1r | 1 | 3 | ✅ done + fetched | 20260524T202219+0800 |
| 4 | ybdb | 3s3r | 3 | 3 | ✅ done + fetched | 20260525T031918+0800 |
...
執行順序（已議定）：**ybdb 4 cells → tidb 4 cells → crdb 4 cells**，每 cell ~3.5h，總剩餘 ~28h（8 cells，ybdb 4 已完成）。
...
### YugabyteDB（cells 1–4，deploy target id `ybdb`）
...
### CockroachDB（cells 9–12，deploy target id `crdb`）
...
- vm-1node CockroachDB 已通過 baseline；vm-3node 同 deploy 路徑，**未踩過 cluster 設定 bug**
...
## 下一步順序（ybdb 4 cells 已完成）
1. dispatch cell 5 (tidb 1s1r)：`make vm3-tidb-1s1r-rc EXECUTE=1 TPCC_TS=$(date '+%Y%m%dT%H%M%S%z')`
2. 等 cell 5 done → fetch → dispatch cell 6 → 依序到 cell 8 (tidb 4 cells)
3. 寫 tidb 4 cells 彙整 record
4. dispatch cell 9 (crdb 1s1r) → 依序到 cell 12 (crdb 4 cells)
5. 全 12 cells 完 → 寫 vm-3node S-BASE 全表彙整 record

---

## ybdb 4 cells baseline（128 wh，5-round mean）

詳見 [results/dispatch-records/2026-05-25-vm-3node-ybdb-all4-rc-analysis.md](./2026-05-25-vm-3node-ybdb-all4-rc-analysis.md)。代表點摘要（throughput-latency 平衡）：

| cell | RF | shards | 代表 threads | tpmC | NO_p99 ms |
|------|---:|------:|:------------:|-----:|----------:|
...
其他 8 cells（tidb 4 + crdb 4）baseline 待完成後彙整。
