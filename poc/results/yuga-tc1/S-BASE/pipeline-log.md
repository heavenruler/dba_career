...
> 本檔為 PoC v4.7 框架下的 YugabyteDB baseline（v4.7 detached suite：5-round × 5 min × 4 thread groups + 20 min warmup × DB-host 雙邊監控）。pre-v4.7 single-run wrapper 資料（vm-1node 單次 10min、k8s-3node-{unlimit,limit} 單次、2.20 issue 紀錄、AlmaLinux 8.10 改造紀錄、原 pipeline-log.md / archive/pipeline-log_old.md）已整理為 [`../../yuga-tc1-old/S-BASE/pipeline-log.md`](../../yuga-tc1-old/S-BASE/pipeline-log.md)；raw wrapper 產物與歷史附件仍保留於 [`../../archive/yuga-tc1-old/`](../../archive/yuga-tc1-old/)，本檔不再保留 pre-v4.7 段落。
