...
| YugabyteDB | 單節點虛擬機，READ COMMITTED / REPEATABLE READ | **11,436** | vm-1node-rc / rr v4.7 完成；rr 為 snapshot iso，撞 retry storm、tpmC -84% vs rc + N-1 errors/round；strict / vm-3node / Kubernetes 待測；pre-v4.7 single-run 與 K8s 歷史已備份於 [yuga-tc1-old/](./yuga-tc1-old/) | [流程紀錄](./yuga-tc1/S-BASE/pipeline-log.md) |
...
| YugabyteDB | 單節點虛擬機 | ✅ 完成 | ✅ 完成 | ⏳ 待執行 | rc peak 11,436 / rr peak 1,879（snapshot iso、retry storm、-84% vs rc、線性 N-1 errors）；strict 待測 |
...
- **vm-1node-rc**：peak **11,436 tpmC @ t32**（5-round mean），零 NEW_ORDER_ERR / 20 round；CPU-bound 含異常高 %sys (19%) — YSQL postgres ↔ DocDB tserver 跨進程 RPC 拉走 1/5 CPU；對比 TiDB %sys 9% / CRDB %sys 5.5%；t128 已過飽和（-4.8% tpmC、p99 翻倍至 1000ms）。
- **vm-1node-rr**：peak **1,879 tpmC @ t32**（5-round mean），比 rc **-84%**；YBDB rr = snapshot isolation（非 PG 標準 RR）撞 SI hot row → 每 round **線性 N − 1 errors**（t16=15 / t32=31 / t64=63 / t128=127）— 與 CRDB rr **完全相同 pattern**，是兩家 optimistic SI 共通病。DB-host %idle 66-67% 全程，瓶頸**不在資源層、在 transaction coordination layer**（retry storm）。三家 RR 對比：TiDB rr 13,874 ＞ CRDB rr 3,788 ＞ YBDB rr 1,879；唯 TiDB pessimistic 可承受 hot-row contention。
- 詳見 [yuga-tc1 pipeline-log TL;DR](./yuga-tc1/S-BASE/pipeline-log.md#tldr--vm-1node-rc--vm-1node-rr-完成2026-05-20--21)。
- vm-1node pre-v4.7 single-run / vm-3node-direct / vm-3node-HAProxy / K8s（unlimit & limit）等歷史結果已備份於 [`yuga-tc1-old/`](./yuga-tc1-old/)，narrative 在 [`yuga-tc1-old/S-BASE/pipeline-log_old.md`](./yuga-tc1-old/S-BASE/pipeline-log_old.md)；v4.7 strict / vm-3node / Kubernetes 重跑尚未排程。
...
| N6 | CRDB / TiDB / YugabyteDB vm-1node-rc + rr 的 tpmC 與 latency p50/p95/p99 已全為 5-round mean；YBDB strict 待測。 |
