...
CAP 說的是系統層在一致性與可用性之間怎麼取捨；`READ COMMITTED` 說的是交易層在一致性與吞吐之間怎麼取捨。這份報告的對標前提是高併發 OLTP 與穩定可用性優先，因此分散式資料庫以 `READ COMMITTED` 作為比較基準。若目標改成更強的交易內一致性，就要接受更高的協調成本與較低的吞吐彈性。
