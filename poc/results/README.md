...
[![YugabyteDB Layered Architecture](https://docs.yugabyte.com/images/architecture/layered-architecture.png)](https://docs.yugabyte.com/stable/architecture/)
...
來源：
- [Architecture — YugabyteDB Docs](https://docs.yugabyte.com/stable/architecture/)（layered，呼應下方 YSQL / YCQL / DocDB / YB-TServer / YB-Master 分層說明）
- [yugabyte/yugabyte-db — Architecture](https://github.com/yugabyte/yugabyte-db#architecture)（整體 component 圖）
...
| <a id="note-3"></a>註3 | **資料品質口徑**：本表所有 v4.7 結果均為 5-round mean（R1-R5 全部納入，對齊 `summary.json` 的 `thread_results.<N>.tpmC_mean`），由 [`tests/common/summary-from-stdout.py`](../tests/common/summary-from-stdout.py) 解析 stdout 後落地至各 suite `summary.json`；pre-v4.7 single-run 結果已封存並整理至 `archive/yuga-tc1-old/`，不納入本表。 |
...
| N4 | v4.7 標準格式：20 分鐘 warmup、每個併發水位 5 round × 5 分鐘、4 thread groups（16/32/64/128）、DB-host 雙邊 OS 監控（mpstat/iostat/vmstat/sar）。TiDB / CockroachDB / YugabyteDB vm-1node 三家全 iso 已採此格式；YugabyteDB vm-3node direct RC 4 cells 已採此格式；YugabyteDB HAProxy 3s3r RC 已完成但 DB-host metrics missing；Kubernetes 重跑尚未排程；YugabyteDB pre-v4.7 single-run 已封存並整理至 `archive/yuga-tc1-old/`。 |
...
- CockroachDB 歷史資料（已封存）：[`archive/cockroach-tc1/S-BASE/archive/pipeline-log.md`](./archive/cockroach-tc1/S-BASE/archive/pipeline-log.md)
- YugabyteDB pre-v4.7 歷史主檔（已封存）：[`archive/yuga-tc1-old/S-BASE/archive/pipeline-log.md`](./archive/yuga-tc1-old/S-BASE/archive/pipeline-log.md)
