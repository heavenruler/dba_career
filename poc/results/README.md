...
|---|---|---|---|---|
| 專案第 1 週 | 2026-05-06 ~ 2026-05-12 | 🟢 | 建立結果索引、統一命名與資料口徑、整理既有 v4.7 baseline 框架 | - |
| 專案第 2 週 | 2026-05-13 ~ 2026-05-19 | 🟢 | 完成 vm-1node 三家三 isolation baseline、整理 README 主表與註解口徑 | - |
| 專案第 3 週 | 2026-05-20 ~ 2026-05-26 | 🟢 | 推進 vm-3node direct / HAProxy、補齊 shard / replica / triple gate 說明、整理 dispatch analysis | - |
| 專案第 4 週 | 2026-05-27 ~ 2026-06-02 | 🟡 | 收斂三節點與 Kubernetes 的待重跑項、整理可移交版本、補強 caveat 與風險結論 | - |
|  |  | 🟡 | 跨專線的分散式資料庫測試項目：初版範圍已定，A/A、A/A(Read Only)、A/S、備份架構與資料遷移策略待補齊 | - |
| 專案第 5 週 | 2026-06-03 ~ 2026-06-09 | ⚪ | 彙整建置 / 維運 / 擴展成本，整理最低成本啟動路徑與前置條件 | - |
| 專案第 6 週 | 2026-06-10 ~ 2026-06-16 | ⚪ | 完成三份報告的整合版、補足缺口與簽核材料 | - |

### 跨專線的分散式資料庫測試項目

> 先用初版把範圍鎖住，後續再依 PoC 實作補齊驗證路徑與量測指標。

| 目前進度 | 測試項目 | 內容 |
|---|---|---|
| 🟢 | 範圍定義 | 先確認跨專線測項要涵蓋的資料庫、拓樸、應用情境與風險邊界，避免混入一般 VM / Kubernetes 基準測試。 |
| 🟡 | A/A 架構 | 雙活架構初版，定義跨專線節點分布、流量分攤、切換條件與資料一致性假設。 |
| 🟡 | A/A(Read Only) 架構 | 讀寫分離或讀為主的雙活初版，定義 read-only 路徑、讀流量分派與 failover 行為。 |
| 🟡 | A/S 架構 | 主備架構初版，定義主節點故障後的切換、回切與資料同步策略。 |
| 🟡 | 備份架構與方式 | 跨專線備份、備份保留、還原流程與驗證方式。 |
| 🟡 | 資料遷移策略 | 初始搬遷、增量同步、切換窗口與回復方案。 |
| ⚪ | 驗證與量測 | 跨專線延遲、故障切換、恢復時間、一致性與可用性測試。 |
| ⚪ | 成本評估 | 建置、維運、擴展與跨專線流量成本，整理最低成本啟動策略。 |
