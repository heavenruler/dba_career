...
> per-round tpmC + 5-round mean（W=128、RC、N=5）。`range/mean` = (max − min) / mean。p99 為 NEW_ORDER 5-round latency mean。tpmTotal / efficiency / p50 / p95 等補充指標見 [`summary.json`](./vm-1node-rc/crdb-vm-1node-rc-20260519T085346+0800/summary.json)。
...
> per-round tpmC + 5-round mean（W=128、RR / SI、N=5）。p99 為 NEW_ORDER 5-round latency mean；err 為 NEW_ORDER `error_rate_pct`（5-round mean）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-1node-rr/crdb-vm-1node-rr-20260519T124506+0800/summary.json)。
...
> per-round tpmC + 5-round mean（W=128、strict / SSI、N=5）。p99 為 NEW_ORDER 5-round latency mean；err 為 NEW_ORDER `error_rate_pct`（5-round mean）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-1node-strict/crdb-vm-1node-strict-20260519T164057+0800/summary.json)。
...
> per-round tpmC + 5-round mean（W=128、RC、N=5）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-3node-1s1r-rc/crdb-vm-3node-1s1r-rc-20260601T105859+0800/summary.json)。
...
> per-round tpmC + 5-round mean（W=128、RC、N=5）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-3node-1s3r-rc/crdb-vm-3node-1s3r-rc-20260601T142702+0800/summary.json)。
...
代表點 = **t=32 / 10,911 tpmC / NO_p99 = 222 ms**（sweet spot）。對照 1s1r（同 1-shard、RF=1）→ **−25.1% throughput / +26% NO_p99**，量化 Raft 3-replica 寫入 quorum amplification（與 YugabyteDB 1s3r 量到的 -25.5% 高度吻合）。t=16 / t=64 range/mean 偏高（16-22%），表示 1s3r 在低或中併發下 Raft commit 路徑波動較大；t=32 / t=128 較穩。
...
> per-round tpmC + 5-round mean（W=128、RC、N=5）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-3node-3s1r-rc/crdb-vm-3node-3s1r-rc-20260601T221341+0800/summary.json)。
...
代表點 = **t=64 / 14,051 tpmC / NO_p99 = 379 ms**（sweet spot）。對照 1s1r（同 RF=1）→ **−3.5% throughput**（CockroachDB sharding 純成本約 4%，遠低於 YugabyteDB 的 -13%；推測 CockroachDB range-leaseholder gateway routing 比 tablet 協調更有效率）。t=128 range/mean 17% 偏高，表示 128 thread 撞到 leaseholder 路由協調瓶頸。
...
> per-round tpmC + 5-round mean（W=128、RC、N=5）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-3node-3s3r-rc/crdb-vm-3node-3s3r-rc-20260602T014253+0800/summary.json)。
...
> per-round tpmC + 5-round mean（W=128、RC、N=5）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-3node-haproxy-3s3r-rc/crdb-vm-3node-haproxy-3s3r-rc-20260602T051500+0800/summary.json)。
