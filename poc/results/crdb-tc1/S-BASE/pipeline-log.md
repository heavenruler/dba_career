...
> 本檔為 PoC v4.7 框架下的 CockroachDB baseline。舊版（cockroach-tc1）資料已整理至 `../../archive/cockroach-tc1/S-BASE/archive/pipeline-log.md`，與本檔流程不同（手動部署、無 detached suite wrapper、無 DB-host 雙邊監控），不直接對比。
...
> 代表點採 t=128；p99 為 NEW_ORDER 5-round latency mean；err 為 all transaction error rate。

| iso | TPCC_TS | 代表併發 | tpmC mean | range/mean | NO p99 mean (ms) | err | N | 判讀 |
|---|---|---:|---:|---:|---:|---:|---:|---|
| [`rc`](#vm-1node-rc--2026-05-19poc-v47-baseline含-db-host-os-監控) | [`20260519T085346`](./vm-1node-rc/crdb-vm-1node-rc-20260519T085346+0800/) | 128 | 8,813 | 4.7% | 926 | 0.000% | 1 | RC；IO-bound |
| [`rr`](#vm-1node-rr--2026-05-19poc-v47crdb-preview-rr) | [`20260519T124506`](./vm-1node-rr/crdb-vm-1node-rr-20260519T124506+0800/) | 128 | 3,788 | 21.2% | 487 | 0.300% | 1 | preview SI；retry storm |
| [`strict`](#vm-1node-strict--2026-05-19poc-v47crdb-serializable--ssi) | [`20260519T164057`](./vm-1node-strict/crdb-vm-1node-strict-20260519T164057+0800/) | 128 | 10,456 | 7.3% | 477 | 0.108% | 1 | SERIALIZABLE / SSI；本組最高 |
...
> per-round tpmC + 5-round mean（W=128、RC、5 rounds）。`range/mean` = (max − min) / mean。p99 為 NEW_ORDER 5-round latency mean。tpmTotal / efficiency / p50 / p95 等補充指標見 [`summary.json`](./vm-1node-rc/crdb-vm-1node-rc-20260519T085346+0800/summary.json)。

| threads | r1 | r2 | r3 | r4 | r5 | mean | range/mean | NO p99 mean (ms) | err |
|--------:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 16  | 9,303 | 9,243 | 9,398 | 8,577 | 8,650 | **9,034.2** | 9.1 % | 111.6 | 0.0 % |
| 32  | 9,110 | 9,215 | 8,679 | 8,973 | 9,125 | 9,020.1 | 5.9 % | 223.1 | 0.0 % |
| 64  | 9,071 | 9,442 | 9,233 | 8,873 | 9,053 | 9,134.4 | 6.2 % | 439.6 | 0.0 % |
| 128 | 8,747 | 8,972 | 8,927 | 8,861 | 8,557 | **8,812.6** | 4.7 % | 926.1 | 0.0 % |
...
### Gate 結果（同 rc，僅 isolation 改 RR）

- `transaction_isolation = repeatable read`（prepare 前 + 後雙閘驗證一致；artifact `gate/isolation-db.txt` + `gate/isolation-driver-verify.txt`）
- 其他 OS gate（THP / swappiness / ulimit / NTP / disk）同 vm-1node-rc

### Prepare

- 時間：43min（128W，與 rc 同流程；prepare 不受 isolation 影響）
- check-all 128 warehouse 全條件通過
- CockroachDB CREATE STATISTICS 9 個統計集建立（與 rc 段相同）

### Execute 結果

> per-round tpmC + 5-round mean（W=128、RR / SI、5 rounds）。p99 為 NEW_ORDER 5-round latency mean；err 為 NEW_ORDER `error_rate_pct`（5-round mean）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-1node-rr/crdb-vm-1node-rr-20260519T124506+0800/summary.json)。

| threads | r1 | r2 | r3 | r4 | r5 | mean | range/mean | NO p99 mean (ms) | err |
|--------:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 16  | 3,142 | 3,353 | 3,542 | 3,144 | 2,961 | **3,228.7** | 18.0 % |  54.1 | 0.093 % |
| 32  | 3,488 | 3,499 | 3,999 | 3,692 | 3,204 | 3,576.6 | 22.2 % | 102.4 | 0.173 % |
| 64  | 3,400 | 3,334 | 3,488 | 3,384 | 4,363 | 3,593.8 | 28.7 % | 216.4 | 0.350 % |
| 128 | 3,976 | 3,502 | 4,305 | 3,644 | 3,515 | **3,788.3** | 21.2 % | 486.5 | 0.666 % ⚠️ |

> ⚠️ t=128 NEW_ORDER err 0.666 % 超過 0.5 % 內部容忍，由 SI WriteTooOldError retry storm 造成（詳見後 starting-gun 段）；err TPM 隨 thread 線性 ~0.2 err/sec/thread。
...
```
threads:    16 ───── 32 ───── 64 ───── 128
tpmC:      3,229   3,577   3,594   3,788
                   +10.8%  +0.5%   +5.4%       ← 整體 flat ~3,600，t128 為 ceiling

p99 (ms):    54     102     216     487
                   +89%    +112%   +125%       ← latency 隨 thread 翻倍

DB %idle:  54.09% 50.70% 53.06% 45.94%        ← DB 大量 idle（46-54% 全程）
DB %iowait: 10.13% 11.36% 10.36% 12.37%       ← IO wait 中位、隨 thread 略升
err / round:  15    31     63     127         ← 線性 N−1
```

**結論**：CockroachDB rr 的 tpmC 天花板 ~3,800（4 vCPU 硬體下），DB host 大量 idle（46-54%）+ 中位 iowait → 瓶頸不在 OS 層；err 線性 N−1 + starting-gun storm 證實 retry storm 為真正成因。

### 觀察

- **tpmC 並非完全 flat 但拉升非常有限**：t128 僅比 t16 高 17%，邊際 +5.4%；對比 rc/strict 在 t64-128 也 flat，但 rr 起點即極低。
- **DB host 大量 idle 但 throughput 砍 60%**：與 rc / strict 的 IO-wait / CPU-bound 模式完全不同，硬體 spec 無法解釋；唯一相容假設為 retry storm 把 worker time 浪費在 client 重送階段（DB 反而閒）。
- **error pattern 與 YugabyteDB rr 完全相同 N−1**：兩家 SI 都在 TPC-C district hot row 撞到 first-committer-wins，差異在 YugabyteDB rr 吞吐更低（DocDB IPC overhead）。
- **`efficiency > 100%` 屬正常**：go-tpc 不打 think time，是本 PoC 內部對標的相對指標，**不可與 TPC-C 官網數字直接比**。
...
| Phase | 起 | 訖 | 耗時 |
|-------|-----|------|------|
| gate | 16:43 | 16:44 | <1min |
| prepare (128W + check-all + CREATE STATISTICS + EXPLAIN) | 16:44 | 17:27 | 43min |
| gate-isolation | 17:27 | 17:28 | <1min |
| run (4 thread × 5 round + 20min warmup) | 17:28 | 20:06 | 2h38min |
| collect | 20:06 | 20:07 | <1s |
| **total** | 16:43 | 20:07 | 3h24min |

### Gate 結果（同 rc，僅 isolation 改 SERIALIZABLE）

- `transaction_isolation = serializable`（prepare 前 + 後雙閘驗證一致；artifact `gate/isolation-db.txt` + `gate/isolation-driver-verify.txt`）
- 其他 OS gate（THP / swappiness / ulimit / NTP / disk）同 vm-1node-rc

### Prepare

- 時間：43min（128W，與 rc / rr 同流程；prepare 不受 isolation 影響）
- check-all 128 warehouse 全條件通過
- CockroachDB CREATE STATISTICS 9 個統計集建立（與 rc 段相同）

### Execute 結果

> per-round tpmC + 5-round mean（W=128、strict / SSI、5 rounds）。p99 為 NEW_ORDER 5-round latency mean；err 為 NEW_ORDER `error_rate_pct`（5-round mean）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-1node-strict/crdb-vm-1node-strict-20260519T164057+0800/summary.json)。

| threads | r1 | r2 | r3 | r4 | r5 | mean | range/mean | NO p99 mean (ms) | err |
|--------:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 16  |  7,463 |  8,450 |  7,868 |  8,321 |  7,288 |  7,878.1 | 14.7 % |  54.1 | 0.037 % |
| 32  | 10,383 | 10,886 |  9,239 |  8,680 | 10,485 |  9,934.5 | 22.2 % | 105.7 | 0.060 % |
| 64  | 10,195 | 11,715 | 10,089 | 11,338 | 10,813 | **10,829.9** | 15.0 % | 219.8 | 0.113 % |
| 128 | 10,046 | 10,679 | 10,729 | 10,792 | 10,033 | 10,455.7 |  7.3 % | 476.5 | 0.240 % |
...
```
threads:    16 ───── 32 ───── 64 ───── 128
tpmC:      7,878   9,935  10,830  10,456
                   +26.1%  +9.0%   -3.5%       ← sweet spot 在 t64，t128 略過飽和

p99 (ms):    54     106     220     477
                   +96%    +108%   +117%       ← latency 翻倍但起點低

DB %idle:  33.48% 27.49% 18.93%  9.99%        ← CPU 隨 thread 線性飽和（與 RR 完全不同）
DB %iowait:  8.88% 11.99% 14.40% 17.25%       ← 隨 thread 上升但起點低於 RC
err / round: 14.6  30.2   58.2   121.8        ← ≈ N−1
```

**結論**：CockroachDB strict 的 tpmC 天花板 ~10,800（4 vCPU 硬體下），sweet spot 在 t64；CPU 隨 thread 線性飽和（%idle 33→10%），驗證 strict 在 IO 路徑比 rc 更省（rc 直接撞 fsync IO 牆），有 CPU headroom 可榨。

### 觀察

- **t64 是 sweet spot**：mean 10,830 tpmC、p99 220ms；t128 換 117% latency 倒退 -3.5% tpmC，不划算。
- **CPU 線性飽和、IO wait 隨 thread 上升**：rc 一開始即被 fsync 卡住（%iowait 18% 全程），strict 改走 read-refresh 路徑 → IO 路徑更省，t16-32 還有 CPU 頭可加；t64+ %idle 跌到 <20%，IO wait 才接近 rc 水位。
- **err count 接近 RR 但 throughput 高 3x**：SSI 在 starting-gun storm 中靠 read-refresh 救回部分衝突，server-internal retry 比 client retry 快得多。
- **`efficiency > 100%` 屬正常**：同 rc / rr 註腳。
...
> 代表點採各 sub-topology 的主要觀察併發；完整 per-round thread sweep 見各 sub-topology 的 `Execute 結果` 表。p99 為 NEW_ORDER 5-round latency mean；err 為 all transaction error rate。

| sub-topology | RF / shards | TPCC_TS | 代表併發 | tpmC mean | range/mean | NO p99 mean (ms) | err | N | 判讀 |
|---|---|---|---:|---:|---:|---:|---:|---:|---|
| [`1s1r`](#vm-3node-1s1r-rc) | RF1 / 1 shard | [`20260601T105859`](./vm-3node-1s1r-rc/crdb-vm-3node-1s1r-rc-20260601T105859+0800/) | 32 | 14,564 | 2.6% | 175 | 0.000% | 1 | cluster framework + RF1 scale-out |
| [`1s3r`](#vm-3node-1s3r-rc) | RF3 / 1 shard | [`20260601T142702`](./vm-3node-1s3r-rc/crdb-vm-3node-1s3r-rc-20260601T142702+0800/) | 32 | 10,911 | 2.8% | 222 | 0.000% | 1 | replica 成本 |
| [`3s1r`](#vm-3node-3s1r-rc) | RF1 / 3 shards | [`20260601T221341`](./vm-3node-3s1r-rc/crdb-vm-3node-3s1r-rc-20260601T221341+0800/) | 64 | 14,051 | 10.7% | 379 | 0.000% | 1 | shard 成本 |
| [`3s3r`](#vm-3node-3s3r-rc) | RF3 / 3 shards | [`20260602T014253`](./vm-3node-3s3r-rc/crdb-vm-3node-3s3r-rc-20260602T014253+0800/) | 64 | 11,132 | 3.8% | 473 | 0.000% | 1 | shard + replica 疊加 |
| [`haproxy-3s3r`](#vm-3node-haproxy-3s3r-rc) | RF3 / 3 shards | [`20260602T051500`](./vm-3node-haproxy-3s3r-rc/crdb-vm-3node-haproxy-3s3r-rc-20260602T051500+0800/) | 128 | 15,033 | 6.9% | 718 | 0.000% | 1 | HAProxy 分散 gateway；本組最高 |
...
| Range SPLIT policy | 無 SPLIT；natural 1 range/table | prepare 階段 |
...
> ⚠️ CockroachDB dry-run 的 `replication-factor.txt` 讀的是 `RANGE default`（系統預設），不是 tpcc DB 的實際 zone。tpcc DB zone 在 prepare 階段設定，hard gate 由 `crdb_internal.ranges` 計數驗。

#### Execute 結果（2026-06-01，TS=20260601T105859+0800）

> per-round tpmC + 5-round mean（W=128、RC、5 rounds）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-3node-1s1r-rc/crdb-vm-3node-1s1r-rc-20260601T105859+0800/summary.json)。

| threads | r1 | r2 | r3 | r4 | r5 | mean | range/mean | NO p99 mean (ms) | err |
|--------:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 16  | 13,261 | 12,723 | 12,767 | 12,840 | 13,181 | 12,954.4 | 4.1 % | 89.8 | 0.0 % |
| 32  | 14,559 | 14,757 | 14,375 | 14,712 | 14,417 | **14,563.8** | 2.6 % | 174.5 | 0.0 % |
| 64  | 14,564 | 14,935 | 14,749 | 11,832 | 14,204 | 14,056.7 | 22.1 % ⚠️ | 389.2 | 0.0 % |
| 128 | 13,991 | 14,682 | 14,094 | 14,192 | 14,341 | 14,259.8 | 4.8 % | 778.5 | 0.0 % |

代表點 = **t=32 / 14,564 tpmC / NO p99 = 175 ms**（sweet spot）。對照 vm-1node-rc（**8,813 tpmC**）→ **+65.3% throughput**（3-node Raft 並行寫入 + leaseholder 分散解單機 fsync 瓶頸；對應 vm-1node §saturation 推測獲得驗證）。t=64 range/mean 22% 偏高（r4 = 11,832 拉低），t=128 latency 翻倍但 tpmC 邊際 +1.4%。詳見 [5-cell suite dispatch](../../dispatch-records/2026-06-02-crdb-vm3-5cell-suite-dispatch.md)。
...
> per-round tpmC + 5-round mean（W=128、RC、5 rounds）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-3node-1s3r-rc/crdb-vm-3node-1s3r-rc-20260601T142702+0800/summary.json)。

| threads | r1 | r2 | r3 | r4 | r5 | mean | range/mean | NO p99 mean (ms) | err |
|--------:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 16  | 10,134 |  9,850 |  8,604 |  9,130 |  9,440 |  9,431.5 | 16.2 % ⚠️ | 125.8 | 0.0 % |
| 32  | 10,962 | 10,722 | 10,868 | 11,022 | 10,981 | **10,910.8** | 2.8 % | 221.5 | 0.0 % |
| 64  | 11,531 | 11,338 | 11,627 |  9,253 | 10,092 | 10,767.9 | 22.0 % ⚠️ | 493.3 | 0.0 % |
| 128 | 10,192 | 10,204 | 10,256 | 10,654 | 10,600 | 10,381.2 |  4.4 % | 1,006.6 | 0.0 % |

代表點 = **t=32 / 10,911 tpmC / NO p99 = 222 ms**（sweet spot）。對照 1s1r（同 1-shard、RF=1）→ **−25.1% throughput / +26% NO p99**，量化 Raft 3-replica 寫入 quorum amplification（與 YugabyteDB 1s3r 量到的 -25.5% 高度吻合）。t=16 / t=64 range/mean 偏高（16-22%），表示 1s3r 在低或中併發下 Raft commit 路徑波動較大；t=32 / t=128 較穩。
...
> per-round tpmC + 5-round mean（W=128、RC、5 rounds）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-3node-3s1r-rc/crdb-vm-3node-3s1r-rc-20260601T221341+0800/summary.json)。

| threads | r1 | r2 | r3 | r4 | r5 | mean | range/mean | NO p99 mean (ms) | err |
|--------:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 16  | 12,309 | 12,016 | 11,534 | 11,767 | 12,229 | 11,970.9 |  6.5 % |  95.7 | 0.0 % |
| 32  | 13,738 | 13,763 | 12,655 | 13,349 | 13,841 | 13,469.0 |  8.8 % | 191.2 | 0.0 % |
| 64  | 12,935 | 14,137 | 14,336 | 14,445 | 14,404 | **14,051.4** | 10.7 % | 379.2 | 0.0 % |
| 128 | 12,880 | 11,779 | 13,566 | 14,043 | 14,001 | 13,253.8 | 17.1 % ⚠️ | 832.2 | 0.0 % |

代表點 = **t=64 / 14,051 tpmC / NO p99 = 379 ms**（sweet spot）。對照 1s1r（同 RF=1）→ **−3.5% throughput**（CockroachDB sharding 純成本約 4%，遠低於 YugabyteDB 的 -13%；推測 CockroachDB range-leaseholder gateway routing 比 tablet 協調更有效率）。t=128 range/mean 17% 偏高，表示 128 thread 撞到 leaseholder 路由協調瓶頸。
...
> per-round tpmC + 5-round mean（W=128、RC、5 rounds）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-3node-3s3r-rc/crdb-vm-3node-3s3r-rc-20260602T014253+0800/summary.json)。

| threads | r1 | r2 | r3 | r4 | r5 | mean | range/mean | NO p99 mean (ms) | err |
|--------:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 16  |  9,034 |  9,089 |  9,298 |  7,998 |  8,949 |  8,873.2 | 14.7 % ⚠️ | 137.6 | 0.0 % |
| 32  | 10,508 | 10,574 | 10,821 | 10,628 | 10,623 | 10,630.6 |  2.9 % | 233.2 | 0.0 % |
| 64  | 11,392 | 10,987 | 11,224 | 10,971 | 11,087 | **11,132.2** | 3.8 % | 473.1 | 0.0 % |
| 128 | 10,837 | 10,881 | 11,113 | 11,009 | 10,813 | 10,930.6 |  2.7 % | 952.9 | 0.0 % |

代表點 = **t=64 / 11,132 tpmC / NO p99 = 473 ms**（sweet spot）。對照 3s1r（同 3-shard）→ **−20.8% throughput**，量化 RF=3 寫入 quorum 成本（與 1s1r→1s3r 量到的 -25% 接近，但 sharded 路徑略低）。對照 1s3r（同 RF=3）→ **+2.0% throughput**（sharding 在 RF=3 下有小幅攤平效益）。CockroachDB 3s3r 的 t=32-128 range/mean ≤4%（遠優於 YugabyteDB 3s3r t=16-128 stddev 1,400-2,615 的極不穩），表示 CockroachDB Raft 在 4 vCPU 3-node + 3-shard 仍可穩定運作。
...
> per-round tpmC + 5-round mean（W=128、RC、5 rounds）。p50/p95/tpmTotal 等補充指標見 [`summary.json`](./vm-3node-haproxy-3s3r-rc/crdb-vm-3node-haproxy-3s3r-rc-20260602T051500+0800/summary.json)。

| threads | r1 | r2 | r3 | r4 | r5 | mean | range/mean | NO p99 mean (ms) | err |
|--------:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 16  | 10,480 |  9,934 |  9,970 |  9,981 | 10,198 | 10,112.7 | 5.4 % | 114.1 | 0.0 % |
| 32  | 11,616 | 11,688 | 12,363 | 12,234 | 11,711 | 11,922.3 | 6.3 % | 203.0 | 0.0 % |
| 64  | 13,592 | 13,939 | 13,634 | 12,747 | 13,306 | 13,443.6 | 8.9 % | 375.8 | 0.0 % |
| 128 | 15,383 | 15,204 | 15,236 | 14,995 | 14,348 | **15,033.3** | 6.9 % | 718.0 | 0.0 % |

代表點 = **t=128 / 15,033 tpmC / NO p99 = 718 ms**（sweet spot at scale）。對照 direct 3s3r（同 RF=3、同 3-shard）→ **+37.5% throughput @ t=128**（CockroachDB direct 模式 client 已連 .32，gateway 已具備內部 lease 路由能力，故 HAProxy 收益 +37.5% 比 TiDB / YugabyteDB 的 +78% 小；CockroachDB 真正能釋放 multi-node 並行的場景需要在 single-entry 與 round-robin 之間多測拓樸變數）。Stability：t=16-128 range/mean ≤8.9%，遠優於 direct 3s3r 在 t=128 的 17.1%。
...
> CockroachDB Kubernetes 對照組（unlimit / limit 兩變體）已於 2026-06-09 / 06-11 完成 v4.7 detached suite 取數，數據與分析見 [`../S-K8S/pipeline-log.md`](../S-K8S/pipeline-log.md)。舊版 cockroach-tc1 VM 資料已整理至 [`../../archive/cockroach-tc1/S-BASE/archive/pipeline-log.md`](../../archive/cockroach-tc1/S-BASE/archive/pipeline-log.md)，與本檔 v4.7 流程不同（手動部署、無 detached suite wrapper、無 DB-host 雙邊監控），不直接對比。
