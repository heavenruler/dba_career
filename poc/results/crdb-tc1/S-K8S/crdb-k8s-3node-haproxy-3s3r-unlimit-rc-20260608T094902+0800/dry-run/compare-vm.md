...
Sources:
- actual: /tmp/poc-tpcc/artifacts/S-K8S/crdb-k8s-3node-haproxy-3s3r-unlimit-rc-20260608T094902+0800/dry-run/actual.yaml
- vm baseline: /tmp/poc-tpcc/phase-k8s/expected/vm-3node-haproxy-3s3r-crdb.yaml
...
## ✅ ALLOW (platform diff)
- .k8s.cluster_name: k8s=cockroachdb vm=<absent>
- .k8s.db_image_prefix: k8s=cockroachdb/cockroach vm=<absent>
- .k8s.namespace: k8s=cockroach-tc1 vm=<absent>
- .k8s.pod_replicas.pd: k8s=0 vm=<absent>
- .k8s.pod_replicas.sql: k8s=3 vm=<absent>
- .k8s.pod_replicas.storage: k8s=3 vm=<absent>
- .k8s.pv_size.storage: k8s=30Gi vm=<absent>
- .k8s.storage_class: k8s=local-path vm=<absent>
- .network.haproxy_backends: k8s=0 vm=3
- .network.nodeport: k8s=30007 vm=<absent>
- .split.strategy: k8s=crdb_split_placeholder vm=crdb_auto_range_split
