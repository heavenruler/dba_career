...
### 目的
驗證 vm-3node 128t（首測 21,875）是否為「16/32/64t 連跑後 cache warm 累積灌水」造成的虛高，獨立跑 128t 從 fresh state 比較。
...
### 環境
- 拓撲：k3s v1.29.14 三節點（.32 master，.33/.34 worker）+ TiDB Operator + TidbCluster (PD×3 / TiKV×3 / TiDB SQL×2)
- TidbCluster `tidb-poc`：PD 10Gi PV、TiKV 100Gi PV、TiDB 無 PV（無狀態）
- 容器資源限制：**無**（unlimit variant）
- 連線入口：NodePort `.32:30004` → tidb-poc-tidb Service → TiDB SQL pods (.32/.33)
