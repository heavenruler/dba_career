...
> 依 PoC-DESIGN §5.4 注意 2：**TiDB rr 與 strict 完全等價**。本輪 vm-1node-rr 已涵蓋同設定下的測試表現，strict 重跑無資訊增量，故略過；待 CRDB / YBDB 等原生 SERIALIZABLE 的 DB 再做 strict 矩陣比較。
...
→ strict 矩陣比較時，**TiDB strict 不可與 CRDB / YBDB strict 直比**；以 `vm-1node-rr` 的數據作為 TiDB 在 strictest native isolation 下的代表即可。
