...
### Execute 結果總覽（vm-1node 兩 isolation）

> 代表點採 t=128；p99 為 NEW_ORDER 5-round latency mean；err 為 all transaction error rate。TiDB strict 不另跑，工具鏈上等同 RR。

| iso | TPCC_TS | 代表併發 | tpmC mean | range/mean | NO p99 mean (ms) | err | N | 判讀 |
|---|---|---:|---:|---:|---:|---:|---:|---|
| [`rc`](#vm-1node-rc--2026-05-18poc-v47-baseline含-db-host-os-監控) | [`20260518T202009`](./vm-1node-rc/tidb-vm-1node-rc-20260518T202009+0800/) | 128 | 13,064 | 8.3% | 597 | 0.000% | 1 | pessimistic RC；零 error |
| [`rr`](#vm-1node-rr--2026-05-19poc-v47含-db-host-os-監控) | [`20260519T001949`](./vm-1node-rr/tidb-vm-1node-rr-20260519T001949+0800/) | 128 | 13,874 | 2.8% | 503 | 0.000% | 1 | pessimistic RR；本組最高 |
| [`strict`](#vm-1node-strict--略過tidb-不支援-serializable) | — | — | — | — | — | — | — | 不執行；等同 RR |
...
per-round tpmC + 5-round mean（W=128、RC、5 rounds）：
...
per-round tpmC + 5-round mean（W=128、RR，5 rounds）：
...
|---|---|---|---:|---:|---:|---:|---:|---:|---|
| [`1s1r`](#vm-3node-1s1r-rc) | RF1 / 1 shard | [`20260529T132940`](./vm-3node-1s1r-rc/tidb-vm-3node-1s1r-rc-20260529T132940+0800/) | 128 | 19,654 | 7.0% | 456 | 0.000% | 1 | cluster framework + RF1 scale-out |
| [`1s3r`](#vm-3node-1s3r-rc) | RF3 / 1 shard | [`20260530T162428`](./vm-3node-1s3r-rc-pd-sched-l4r4/tidb-vm-3node-1s3r-rc-20260530T162428+0800/) | 128 | 16,336 | 1.6% | 527 | 0.000% | 1 | replica 成本 |
| [`3s1r`](#vm-3node-3s1r-rc) | RF1 / 3 shards | [`20260530T023238`](./vm-3node-3s1r-rc/tidb-vm-3node-3s1r-rc-20260530T023238+0800/) | 64 | 16,580 | 15.6% | 270 | 0.000% | 1 | shard 成本 |
| [`3s3r`](#vm-3node-3s3r-rc) | RF3 / 3 shards | [`20260531T085812`](./vm-3node-3s3r-rc-pd-sched-l4r4/tidb-vm-3node-3s3r-rc-20260531T085812+0800/) | 128 | 15,082 | 11.6% | 591 | 0.000% | 1 | shard + replica 疊加 |
| [`haproxy-3s3r`](#vm-3node-haproxy-3s3r-rc) | RF3 / 3 shards | [`20260601T003316`](./vm-3node-haproxy-3s3r-rc-pd-sched-l4r4/tidb-vm-3node-haproxy-3s3r-rc-20260601T003316+0800/) | 128 | 26,947 | 7.4% | 309 | 0.000% | 1 | HAProxy 分散 SQL entry；本組最高 |
...
代表點 = **t=64 / 16,580 tpmC / NO p99 = 270 ms**（mean tpmC 最大且不撞極端 latency 之平衡點；t=128 latency 翻倍且 range/mean 飆 23.8%，r1=14,515 vs r2=17,949 為主要震盪源）。對照 vm-3node-1s1r（同 RF=1）→ **−15.6% throughput**（t=128 對比），量化 TiDB 在 RF=1 下純 sharding 成本（low 於 YugabyteDB 同位 −12.7%）。t=64 range/mean 15.6% 偏高，可能與 region leader 分布有關。
