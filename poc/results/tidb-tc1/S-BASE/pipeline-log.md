...
k8s-3node-unlimit                         client (go-tpc on .31)
                              ...
                              │ └──────────────┘                  │
                              └───────────────────────────────────┘
  deploy: tidb-k8s.yml + tidb-tc1-k8s.ini + TidbCluster CR (k8s_resource_limits=false)
...
TiDB 三節點 + HAProxy 是 **OLTP 高併發場景的最佳部署模式**。clean 重跑 peak 22,841 tpmC 為單節點 13,355 的 **1.71×**，超越 CRDB 同等部署 1.63×。  
...
K8s 部署的 TiDB 比 VM bare-process 部署 **慢約 12-17%**（高併發更明顯），但仍遠優於 CRDB（14,014）和 YBDB（1,036）的 VM 部署。
若選 K8s 為部署模式，需留意：
