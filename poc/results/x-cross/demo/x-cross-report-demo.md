...
| D3 | Leader / replica 實際位置符 Placement Policy | **TiDB + CRDB DONE / YBDB DEFERRED**：`tests/common/prepare.sh` §6.6 `placement leader-distribution gate` (vm-6node-* fail-closed)；P-A 預期 IDC ≥ 70%；P-B 預期 30%-70% spread；FAIL → 中斷 prepare 不進 timed run；artifact `placement-gate-<P-A\|P-B>.json` (verdict + reason + counts)。TiDB 用 TIKV_REGION_PEERS + JSON_CONTAINS；CRDB 用 SHOW RANGES FROM TABLE WITH DETAILS + lease_holder_locality grep；YBDB 需 SSH+yb-admin，留 framework patch 委派到 gate-placement-*.sh | 0630.md §9.2 + decisions Q11/Q12 |
...
| D6 | 就近讀真生效（routing 證據） | **IDC pipeline hook DONE**：`tests/common/run.sh` per-round（vm-6node-* gated）自動 spawn `probe-iso-latency.sh --label idc-t<N>-r<R>`；artifact 寫進 `runs/threads-<N>/round-<R>/probe-iso-latency-idc-*.{csv,json}`。**GCP side TODO**：需 deploy probe script + DB client 到 g-test-poc-5；framework patch 階段補（同次 ssh g-test-poc-5 拉 label=gcp）。比 p99：GCP SELECT 1 ~50ms 即 follower read 沒生效 | 0630.md §9.3 + decisions Q13 |
