...
| D3 | Leader / replica 實際位置符 Placement Policy | **TiDB DONE**：`tests/common/prepare.sh` §6.6 `placement leader-distribution gate` (vm-6node-* fail-closed)；P-A 預期 IDC ≥ 70%；P-B 預期 30%-70% spread；FAIL → 中斷 prepare 不進 timed run；artifact `placement-gate-<P-A\|P-B>.json` (verdict + reason + counts)。CRDB/YBDB Q13 PLANNED 同章節 TODO | 0630.md §9.2 + decisions Q11/Q12 |
