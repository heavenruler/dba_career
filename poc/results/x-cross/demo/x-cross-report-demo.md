...
| D6 | 就近讀真生效（routing 證據） | **TOOL READY**：`tests/common/probe-iso-latency.sh` standalone — `--db {tidb/crdb/ybdb} --duration-sec N --label {idc/gcp}` 跑 SELECT 1 + UPDATE 取 p50/p95/p99；同次 timed run 期間從 IDC (.31) 與 GCP (g-test-poc-5) 各 call 一次 → 比 p99；GCP SELECT 1 ~50ms 即 follower read 沒生效。Pipeline hook 待 framework patch | 0630.md §9.3 + decisions Q13 |
