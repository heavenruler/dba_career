...
| 6 | Chaos C1 / C4 spec ↔ planner script 名稱錯置 | `chaos/C1.md` spec = WAN partition iptables；`chaos/C4.md` spec = IDC leader die systemctl stop。原 planner `chaos-c1-node-down-plan.sh` model = systemctl stop（C4 故障模型），`chaos-c4-network-partition-plan.sh` model = iptables（C1 故障模型）— 名稱與內容互換 | **已修（audit-6 2026-06-30）**：rename + 內容重寫：`chaos-c1-partition-plan.sh`（WAN partition）/ `chaos-c4-node-down-plan.sh`（IDC leader die）| SSOT 與 script 一致 |
...
| 11 | 就近讀寫 routing evidence + control plane cost 量測 — **三家 prepare SET + 三家 placement gate + summary.json region_routing_evidence + probe-iso-latency.sh + IDC+GCP probe hook + netflow-snapshot 全部落地**；只剩 bytes-level traffic + AS OF SYSTEM TIME 整合 | per `1_MeetingMinutes/0630.md` §6 + §9：data plane 設定 + PD metadata/TSO cost + Grafana KV Request routing + NetFlow per-port 分類；TL;DR §A 比值在 routing 證據齊全前為 architectural inference 非 measurement | **(已落地)** prepare §6.5 三家 SET；§6.6 三家 placement gate；summary.json region_routing_evidence；probe-iso-latency.sh standalone + IDC+GCP pipeline hook；`tests/common/netflow-snapshot.sh` ss-based per-host per-port connection count + run.sh pre-run/post-run snapshot hook。**(仍缺)** (a) iptables counter bytes 量測（connection count ≠ bytes throughput）；(b) CRDB AS OF SYSTEM TIME 套到 go-tpc / YBDB 連線重連 | TBD（與 codex F1-F8 同批做） |
