...
| 11 | 就近讀寫 routing evidence + control plane cost 量測 — **TiDB prepare 已部分落地**（2026-06-30 commit）；其餘 4 路徑仍缺 | per `1_MeetingMinutes/0630.md` §6 + §9：data plane closest-replicas 設定 + PD metadata/TSO cost + Grafana KV Request routing + NetFlow per-port 分類；TL;DR §A 比值在 routing 證據齊全前為 architectural inference 非 measurement | **(已落地)** `tests/common/prepare.sh` §6.5 vm-6node-* + DB=tidb 時 SET `tidb_replica_read='closest-replicas'` + `pd_enable_follower_handle_region=ON` 並 dump `near-read-vars.txt`。**(仍缺)** (a) CRDB / YBDB 等價 SET（Q13 PLANNED）；(b) dump-actual 0630.md §9.2 leader 分布 SQL；(c) collect 加 NetFlow port 分類 + IDC vs GCP 同 SQL latency 對比；(d) summary.json schema 加 `region_routing_evidence` | TBD（與 codex F1-F8 同批做） |
