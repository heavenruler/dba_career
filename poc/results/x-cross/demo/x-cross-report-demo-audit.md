...
| 11 | 就近讀寫 routing evidence + control plane cost 量測 — **三家 prepare SET + TiDB & CRDB placement gate + summary.json region_routing_evidence 已落地**；YBDB gate + 量測 2 路徑仍缺 | per `1_MeetingMinutes/0630.md` §6 + §9：data plane 設定 + PD metadata/TSO cost + Grafana KV Request routing + NetFlow per-port 分類；TL;DR §A 比值在 routing 證據齊全前為 architectural inference 非 measurement | **(已落地)** `tests/common/prepare.sh` §6.5 三家自動 SET；§6.6 TiDB+CRDB placement gate（fail-closed）；**`summary-from-stdout.py` 加 `region_routing_evidence` 欄**：cell summary.json 自動內嵌 near_read_setup（vars_snapshot）+ placement_gate（verdict/placement/idc_leader_count/gcp_leader_count/total_leader_samples）。**(仍缺)** (a) YBDB §6.6 等價 gate（需 SSH+yb-admin）；(b) collect 加 NetFlow port 分類 + IDC vs GCP 同 SQL latency 對比；(c) CRDB 查詢層 `AS OF SYSTEM TIME` 是否套用到 go-tpc / YBDB 既有連線重連策略 | TBD（與 codex F1-F8 同批做） |
