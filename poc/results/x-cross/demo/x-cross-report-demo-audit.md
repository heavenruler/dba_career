...
| 11 | 就近讀寫 routing evidence + control plane cost 量測 — **三家 prepare SET + TiDB & CRDB placement gate 已落地**；YBDB gate + 量測 3 路徑仍缺 | per `1_MeetingMinutes/0630.md` §6 + §9：data plane 設定 + PD metadata/TSO cost + Grafana KV Request routing + NetFlow per-port 分類；TL;DR §A 比值在 routing 證據齊全前為 architectural inference 非 measurement | **(已落地)** `tests/common/prepare.sh` §6.5 三家自動 SET；**§6.6 TiDB (JSON_CONTAINS TIKV_REGION_PEERS) + CRDB (SHOW RANGES + lease_holder_locality grep) placement leader-distribution gate**（fail-closed：P-A IDC≥70%；P-B 30-70% spread）。**(仍缺)** (a) YBDB §6.6 等價 gate（需 SSH+yb-admin，委派到 gate-placement-*.sh）；(b) collect 加 NetFlow port 分類 + IDC vs GCP 同 SQL latency 對比；(c) summary.json schema 加 `region_routing_evidence`；(d) CRDB 查詢層 `AS OF SYSTEM TIME` 是否套用到 go-tpc 流程 / YBDB 既有連線重連策略 | TBD（與 codex F1-F8 同批做） |
