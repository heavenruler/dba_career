...
| 11 | 就近讀寫 routing evidence + control plane cost 量測 — **三家 prepare SET + 三家 placement gate + summary.json region_routing_evidence + probe-iso-latency.sh + IDC+GCP pipeline hook 全部落地**；NetFlow + AS OF SYSTEM TIME 整合仍缺 | per `1_MeetingMinutes/0630.md` §6 + §9：data plane 設定 + PD metadata/TSO cost + Grafana KV Request routing + NetFlow per-port 分類；TL;DR §A 比值在 routing 證據齊全前為 architectural inference 非 measurement | **(已落地)** prepare §6.5 三家 SET；§6.6 三家 placement gate（TiDB JSON_CONTAINS / CRDB SHOW RANGES / YBDB ssh+yb-admin）；summary.json region_routing_evidence；probe-iso-latency.sh standalone；run.sh §3.5 GCP deploy + per-round IDC+GCP probe sidecar。**(仍缺)** (a) NetFlow port 分類；(b) CRDB AS OF SYSTEM TIME 套到 go-tpc / YBDB 連線重連 | TBD（與 codex F1-F8 同批做） |
