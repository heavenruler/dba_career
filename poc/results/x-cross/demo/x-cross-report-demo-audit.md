...
| 11 | 就近讀寫 routing evidence + control plane cost 量測 — **三家 prepare 機制 SET 已落地**；其餘 3 路徑仍缺 | per `1_MeetingMinutes/0630.md` §6 + §9：data plane 設定 + PD metadata/TSO cost + Grafana KV Request routing + NetFlow per-port 分類；TL;DR §A 比值在 routing 證據齊全前為 architectural inference 非 measurement | **(已落地)** `tests/common/prepare.sh` §6.5 vm-6node-* 三家自動 SET 並 dump artifact (TiDB closest-replicas + pd_enable_follower_handle_region；CRDB kv.closed_timestamp.follower_reads_enabled；YBDB ALTER DATABASE yb_read_from_followers + yb_follower_read_staleness_ms=30000)。**(仍缺)** (a) dump-actual 0630.md §9.2 leader 分布 SQL；(b) collect 加 NetFlow port 分類 + IDC vs GCP 同 SQL latency 對比；(c) summary.json schema 加 `region_routing_evidence`；(d) CRDB 查詢層 `AS OF SYSTEM TIME` 是否套用到 go-tpc 流程 / YBDB 既有連線重連策略 | TBD（與 codex F1-F8 同批做） |
