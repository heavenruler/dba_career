...
| 11 | 就近讀寫 routing evidence + control plane cost 量測 — **三家 prepare SET + TiDB & CRDB placement gate + summary.json region_routing_evidence + probe-iso-latency.sh tool 已落地**；pipeline hook + NetFlow + YBDB gate 仍缺 | per `1_MeetingMinutes/0630.md` §6 + §9：data plane 設定 + PD metadata/TSO cost + Grafana KV Request routing + NetFlow per-port 分類；TL;DR §A 比值在 routing 證據齊全前為 architectural inference 非 measurement | **(已落地)** prepare §6.5 三家 SET；§6.6 TiDB+CRDB placement gate；summary.json region_routing_evidence；**`tests/common/probe-iso-latency.sh` standalone** 三家 latency probe（SELECT 1 + UPDATE p50/p95/p99，dump csv + json）。**(仍缺)** (a) YBDB §6.6 等價 gate；(b) **probe-iso-latency.sh pipeline hook** — 從 run.sh / run-vm6-suite.sh 同次 timed run 啟動 background sidecar 跑 .31 + GCP 兩 label；(c) NetFlow port 分類；(d) CRDB AS OF SYSTEM TIME 套到 go-tpc / YBDB 連線重連 | TBD（與 codex F1-F8 同批做） |
