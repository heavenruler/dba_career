...
| 11 | 就近讀寫 routing evidence + control plane cost 量測 — **三家 prepare 機制 SET 已落地 + TiDB placement leader-distribution gate 已落地**；其餘 3 路徑仍缺 | per `1_MeetingMinutes/0630.md` §6 + §9：data plane 設定 + PD metadata/TSO cost + Grafana KV Request routing + NetFlow per-port 分類；TL;DR §A 比值在 routing 證據齊全前為 architectural inference 非 measurement | **(已落地)** `tests/common/prepare.sh` §6.5 三家自動 SET 並 dump artifact；**§6.6 TiDB placement leader-distribution gate**（fail-closed：P-A 預期 IDC ≥ 70%；P-B 30%-70% spread；FAIL → 中斷 prepare）。**(仍缺)** (a) CRDB/YBDB §6.6 等價 gate（Q13 PLANNED 同章節）；(b) collect 加 NetFlow port 分類 + IDC vs GCP 同 SQL latency 對比；(c) summary.json schema 加 `region_routing_evidence`；(d) CRDB 查詢層 `AS OF SYSTEM TIME` 是否套用到 go-tpc 流程 / YBDB 既有連線重連策略 | TBD（與 codex F1-F8 同批做） |
