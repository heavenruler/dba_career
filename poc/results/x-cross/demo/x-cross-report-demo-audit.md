...
| 11 | 就近讀寫 routing evidence + control plane cost 量測 — **三家 prepare SET + TiDB & CRDB placement gate + summary.json region_routing_evidence + probe-iso-latency.sh tool + IDC+GCP pipeline hook 已落地**；NetFlow + YBDB gate 仍缺 | per `1_MeetingMinutes/0630.md` §6 + §9：data plane 設定 + PD metadata/TSO cost + Grafana KV Request routing + NetFlow per-port 分類；TL;DR §A 比值在 routing 證據齊全前為 architectural inference 非 measurement | **(已落地)** prepare §6.5 三家 SET；§6.6 TiDB+CRDB placement gate；summary.json region_routing_evidence；probe-iso-latency.sh standalone；**run.sh §3.5 idempotent scp 到 g-test-poc-5 + per-round IDC + GCP sidecar 自動 spawn**（vm-6node-* gated）→ artifact `probe-iso-latency-{idc,gcp}-*.{csv,json}`。**(仍缺)** (a) YBDB §6.6 等價 gate（SSH+yb-admin）；(b) NetFlow port 分類；(c) CRDB AS OF SYSTEM TIME 套到 go-tpc / YBDB 連線重連 | TBD（與 codex F1-F8 同批做） |
