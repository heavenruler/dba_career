...
| **WAL durable** | **三家強制 fsync-on-commit** | YugabyteDB 需顯式 `--durable_wal_write=true --require_durable_wal_write=true` |
...
| CockroachDB load-based splitting | **vm-1node 不關**（自然不觸發）；**vm-3node 全部關**（`kv.range_split.by_load_enabled=false`） | controlled experiment 不能讓 split 干擾 shard 鎖定 |
| YugabyteDB automatic tablet splitting | **vm-3node 全部關**（`enable_automatic_tablet_splitting=false`） | 同上 |
...
**CockroachDB 特別聲明**：[官方文件](https://www.cockroachlabs.com/docs/stable/cockroach-start-single-node)明確標註 `start-single-node` 為 single-node cluster with replication disabled，**不適合 performance testing**。本 PoC 採用是為了與 TiDB `max-replicas=1` / YugabyteDB RF=1 對齊對比基準，**不可外推為 CockroachDB production 數字**。
...
| REPEATABLE READ | ✅ 預設（實作為 Snapshot Isolation） | △ opt-in/preview：v24.3+ 加入 `sql.txn.repeatable_read_isolation.enabled`，**預設 false**，需手動開啟；CockroachDB 主述為 SER/RC 兩級 | ✅ 支援（Snapshot Isolation） | △ 三家可用，CockroachDB 為 preview |
...
- CockroachDB: [read-committed docs](https://www.cockroachlabs.com/docs/stable/read-committed) / [v24.3 release notes](https://www.cockroachlabs.com/docs/releases/v24.3)
- YugabyteDB: [read-committed architecture](https://docs.yugabyte.com/preview/architecture/transactions/read-committed/) / [yb-tserver config](https://docs.yugabyte.com/preview/reference/configuration/yb-tserver/)

### 5.3 為何主對標選 READ COMMITTED

| 理由 | 說明 |
|---|---|
| **三家 production-ready** | TiDB v8.5（pessimistic）/ CockroachDB v26.2（預設開啟）/ YugabyteDB v2025.2 yugabyted（預設開啟）皆原生支援，非 client 端假象映射 |
...
| 拓撲 | TiDB | CockroachDB | YugabyteDB |
|---|---|---|---|
...
> 注意 2：**TiDB rr 與 strict 完全等價**（TiDB 不支援 SERIALIZABLE，REPEATABLE READ 已是 native 最強）。本實作將 `tidb:strict` 在 `tests/common/lib/common.sh` alias 到 `tidb:rr`；為避免重跑同設定 ~3.6h 但無資訊增量，**TiDB strict artifact 略過**，後續報表以 `vm-1node-rr` 作 TiDB native strictest 代表（CockroachDB / YugabyteDB 仍跑 strict，因有原生 SSI / SI）。
> 注意 3：**CockroachDB rr 為 preview feature**，需 cluster setting opt-in；行為與 SI/RR 標準語義可能有微差，報告須標 preview 警語。

**預期觀察（探索性假設，僅為設計時參考，非官方 benchmark spec，以實測為準）**：

| 對比 | TiDB | CockroachDB | YugabyteDB |
|---|---|---|---|
...
| sub-topo | shard 數/表 | RF | TiDB | CockroachDB | YugabyteDB |
|---|---:|---:|---|---|---|
...
| 檔案 | 內容 |
|---|---|
| `cluster-topology.txt` | TiDB: `tiup cluster display`；CockroachDB: `cockroach node status`；YugabyteDB: `yb-admin list_all_tablet_servers` |
| `replication-factor.txt` | TiDB: `SHOW CONFIG WHERE NAME='replication.max-replicas'`；CockroachDB: `SHOW ZONE CONFIGURATION FROM DATABASE tpcc`（prepare 前可能還沒 tpcc，改 `defaultdb`）；YugabyteDB: `yb-admin get_universe_config` |
| `cluster-health.txt` | TiDB: `SELECT 1`；CockroachDB: `cockroach node status --ranges`；YugabyteDB: `curl :7000/cluster` |
| `iso-preset.txt` | conn-params probe（同 §7.4 Layer B），確認 `BEGIN; SHOW transaction_isolation;` 與預期一致；YugabyteDB 同檔加 `SELECT yb_get_effective_transaction_isolation_level()`（triple gate active 層），詳見 §7.4「YugabyteDB triple gate」 |
| `expected-vs-actual.txt` | 4 個項目逐項對比預期：node 數、PD/master quorum、RF、iso；YugabyteDB 額外列 triple gate 三層 actual |
...
| CockroachDB | `systemctl stop cockroach.service` + **wait `pgrep -x cockroach` 為空** + `pkill -9` + `rm /etc/systemd/system/cockroach.service` + `rm /data/crdb` | `cockroach start --background` daemonize 後 systemctl loses track；不等 process 真死就 rm，下次 deploy 會帶舊 cluster ID join，出現 `client cluster ID "X" doesn't match server cluster ID "Y"` 死循環 |
| YugabyteDB | `pkill -KILL -x yb-master / yb-tserver / yugabyted-ui` + 對所有 `pgrep -x python3` 讀 `/proc/<pid>/cmdline` 過濾含 `bin/yugabyted` 的才 kill + `rm /var/yugabyte` | yugabyted 是 python3 wrapper；用 `pgrep -f yugabyted` 會自殺（pgrep 自己 cmdline 也含 "yugabyted"） |
...
| 4 | A | vm-1node | CockroachDB | rc | 1 | 1 | 150 min | 7.5 h |
| 5 | A | vm-1node | CockroachDB | rr | 1 | 1 | 150 min | 10.0 h |
| 6 | A | vm-1node | CockroachDB | strict | 1 | 1 | 150 min | 12.5 h |
| 7 | A | vm-1node | YugabyteDB | rc | 1 | 1 | 150 min | 15.0 h |
| 8 | A | vm-1node | YugabyteDB | rr | 1 | 1 | 150 min | 17.5 h |
| 9 | A | vm-1node | YugabyteDB | strict | 1 | 1 | 150 min | **20.0 h ← Phase A 結束** |
| 10 | B | vm-3node-1s1r | TiDB | rc | 1 | 1 | 180 min | 23.0 h |
| 11 | B | vm-3node-1s1r | CockroachDB | rc | 1 | 1 | 180 min | 26.0 h |
| 12 | B | vm-3node-1s1r | YugabyteDB | rc | 1 | 1 | 180 min | **29.0 h ← Phase B 結束** |
| 13 | C | vm-3node-1s3r | TiDB | rc | 1 | 3 | 180 min | 32.0 h |
| 14 | C | vm-3node-1s3r | CockroachDB | rc | 1 | 3 | 180 min | 35.0 h |
| 15 | C | vm-3node-1s3r | YugabyteDB | rc | 1 | 3 | 180 min | **38.0 h ← Phase C 結束** |
| 16 | D | vm-3node-3s1r | TiDB | rc | 3 | 1 | 180 min | 41.0 h |
| 17 | D | vm-3node-3s1r | CockroachDB | rc | 3 | 1 | 180 min | 44.0 h |
| 18 | D | vm-3node-3s1r | YugabyteDB | rc | 3 | 1 | 180 min | **47.0 h ← Phase D 結束** |
| 19 | E | vm-3node-3s3r | TiDB | rc | 3 | 3 | 180 min | 50.0 h |
| 20 | E | vm-3node-3s3r | CockroachDB | rc | 3 | 3 | 180 min | 53.0 h |
| 21 | E | vm-3node-3s3r | YugabyteDB | rc | 3 | 3 | 180 min | **56.0 h ← Phase E 結束** |
| 22 | F | vm-3node-haproxy-3s3r | TiDB | rc | 3 | 3 | 180 min | 59.0 h |
| 23 | F | vm-3node-haproxy-3s3r | CockroachDB | rc | 3 | 3 | 180 min | 62.0 h |
| 24 | F | vm-3node-haproxy-3s3r | YugabyteDB | rc | 3 | 3 | 180 min | **65.0 h ← Phase F 結束** |
...
| DB 啟動穩定（YugabyteDB tablet rebalance、CockroachDB compaction、TiDB Region heartbeat），尤其 RF=3 + 3 shards | ~+5 min/組（vm-3node 15 組） | +1.25 h |
...
> **重要原則**：隔離級**不在 DB 端設 default**（CockroachDB `root` user 對 role-level default exempt；YugabyteDB DB-level default 會被 prepare DROP 洗掉）。改採 **go-tpc connection-string 帶 session 參數** 強制隔離級，並在 run 前以 **active transaction** （`BEGIN; SHOW transaction_isolation; COMMIT;`）驗證實際生效。
> Source（CockroachDB root exemption）: <https://www.cockroachlabs.com/docs/stable/alter-role>
...
> ⚠️ **不用 `ALTER ROLE root SET default_transaction_isolation`**：CockroachDB 官方明說 `root` 對 role/all-role default session settings exempt，只受 connection-string 影響。
...
```
> CockroachDB v24.3+ 提供，預設 false；v26.2 仍為 preview feature。報告須加 preview 警語。
...
**deploy 後**：無 DB 級 isolation 設定（DB-level setting 會隨 `DROP DATABASE` 洗掉）。隔離級控制走 connection-string，與 CockroachDB 同 PostgreSQL `options`。
...
**Isolation 控制：go-tpc connection-string**（與 CockroachDB 同 PostgreSQL options）
...
> YugabyteDB RR 對應 Snapshot Isolation 實作。
...
2. **prepare 完成後手動 split**（9 張表全鎖；§7.5.4）：
   ...
   -- CockroachDB compound PK split 可只填 prefix
   -- 1 shard 不下 SPLIT
   ```
...
**TPC-C schema 共 9 張表，本 PoC 全部鎖定**（取消「不鎖 item / history」的例外，因為 YugabyteDB 3 tservers 預設不會自然 1 shard，且 history 是 Payment append 高頻寫入表）：
...
#### YugabyteDB triple gate（Layer A 的 YugabyteDB 子規則）
...
```bash
# CockroachDB / YugabyteDB (PostgreSQL 協定)
go-tpc tpcc run \
  -d postgres \
...
| SQLSTATE histogram | DB log diff（TiDB tidb.log / CockroachDB cockroach.log / YugabyteDB postgres.log）grep `SQLSTATE=` | 欄位標 **`N/A`**，附 raw log 路徑 |
...
| prepare | **強制** `DROP DATABASE IF EXISTS tpcc; CREATE DATABASE tpcc;` 後 go-tpc prepare | 不接受「row count=0 就跳過 drop」；YSQL/CockroachDB 沒有 `CREATE IF NOT EXISTS` 語法，wrapper 應容忍 `duplicate_database` (SQLSTATE 42P04) error |
...
- 在 `BEGIN..COMMIT` 內 `SHOW transaction_isolation`（CockroachDB 官方要求在 transaction 內看才是 active 值）
- 結果必須**完全等於** `<iso>` 預期；不一致 → fail closed
- 額外輔助驗證：
  - CockroachDB: `SHOW CLUSTER SETTING sql.txn.read_committed_isolation.enabled` = true
  - YugabyteDB: `curl :9000/varz | grep yb_enable_read_committed_isolation|ysql_enable_auto_analyze|durable_wal_write`
...
curl -s http://$YUGABYTE_HOST:9000/tablet-servers
```
...
CockroachDB_HOST         := 172.24.40.32
CockroachDB_PORT         := 26257
CockroachDB_USER         := root
CockroachDB_DB           := tpcc

YugabyteDB_HOST         := 172.24.40.32
...
# CockroachDB / YugabyteDB（PostgreSQL options GUC）
...
# CockroachDB RR tier 需先 opt-in（cluster setting）
CockroachDB_RR_OPTIN     := SET CLUSTER SETTING sql.txn.repeatable_read_isolation.enabled = true;
...
| CockroachDB | ... | ... | ... | ... | ... | SERIALIZABLE | rr 為 preview |
| YugabyteDB | ... | ... | ... | ... | ... | SERIALIZABLE | rr = SI |
```
...
| 隔離成本對照 | 追加 vm-1node-rr 與 vm-1node-strict 兩階 | 量化「升一級 / 升到最強」對 tpmC 影響；TiDB rr ≡ strict（native 最強就是 RR）；CockroachDB rr 為 preview opt-in |
...
| CockroachDB load-based split | 不關 | 官方預設且生產建議；vm-1node 自然不觸發 |
| CockroachDB 密碼設定 | 不在 `--insecure` 下 SET PASSWORD | 官方不支援；用 HBA trust |
| YugabyteDB auto-analyze flag | `ysql_enable_auto_analyze=false` | 2025.2 正確 flag 名 |
| **Isolation 控制方式** | **走 go-tpc `--conn-params`（PostgreSQL `options` GUC / MySQL DSN session vars）** | CockroachDB `root` 對 ALTER ROLE default exempt；YugabyteDB DB-level setting 隨 DROP 洗掉；conn-params 是唯一可靠路徑 |
| Active isolation gate | 在 `BEGIN..COMMIT` 內 `SHOW transaction_isolation` 驗證 | CockroachDB 官方要求 active 值需 transaction 內看；防 default 與 active 不符 |
