...
- 容器資源限制：每個 CRDB pod `requests: 1 CPU / 2GiB`，`limits: 2 CPU / 8GiB`
...
CRDB 對 K8s 容器化本身幾乎無損，但對 CPU limit 非常敏感。若生產環境要用 K8s 跑 CRDB，`limits.cpu=2` 會把 TPC-C peak 壓低約一半；建議 capacity planning 以 CPU limit 為主要 sizing 參數，而不是只看 pod 數。
