...
| # | 內容 | 依據 | 驗收 |
|---|---|---|---|
| M1 | 合入 `5cd6d6d8`（spike/healthcheck-poc）**但埠改 19999**（spike 原寫 20170，D9 已改）：cherry-pick 後把 wan-probe.sh + idc-iperf3-bootstrap.sh 的 IPERF_PORT 20170→19999 | plan S1 + D9 | `grep -c '未開通' scripts/wan-probe.sh`=0；`grep IPERF_PORT scripts/{wan-probe,idc-iperf3-bootstrap}.sh \| grep 19999` |
...
| M8 | **刪除 Path C**（R2 拍板=廢）：warmup-only×3 + roundrun-only×3 + orchestrator(:1268 帶) + run-round-only.sh + README 提及 | R2/plan S2 | `grep -rn 'run-round-only\|warmup-only\|roundrun-only' poc/ --include=Makefile --include='*.md' \| grep -v results/ \| grep -v .bak` 為空 |
| M9 | **拆 main.tf iperf3 常駐 daemon**（R1 拍板）：刪 main.tf:122-139 的 iperf3-server.service 段，保留 binary 安裝；埠 19999 已在 M1 統一 | R1/plan S2b | `grep -c 'enable --now iperf3-server' iac-gcp/main.tf`=0；`grep -c iperf3 iac-gcp/main.tf`>0 |

每步：最小 diff → `bash -n`（tf 用 `terraform fmt -check`）→ 單獨 commit（`fix(phase-crossregion):` 前綴、-F）。
**勿碰**：sweep-archive.sh（S4，非 Wave 4 必經，時間富裕才修）；tests/common/*、tfvars/tfstate（硬規則）。
（原「勿碰」的 Path C 與 main.tf 5201 已於 2026-07-07 拍板改為 M8/M9 處理——見 decisions D9 / risks R1,R2。）
