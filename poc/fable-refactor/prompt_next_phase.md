...
## 現況（2026-07-07 更新，已驗證）
- **master @ edd6d5b4**；VM 全拆（iac-idc/iac-gcp state 0）；每 cell 從 `make phase1` 起（Q11）
...
| M5 | run-vm6-suite.sh ybdb 分支改呼叫 freeze/unfreeze-ybdb.sh | `24a59df5`（**5d live 驗證仍待 YBDB smoke**）|
...
剩餘：**S8**（收斂清理，P2-1/P2-2/P2-4 等）→ **S9**（Win-2 前 PROFILE 命名實作，需與 gap#1 A-A 接線綁做）。
