...
### R1. iperf3 server 架構二選一（P1-4 的根）
`iac-gcp/main.tf` 起 5201 常駐 daemon vs `wan-probe.sh` 用 20170 臨時 server——兩套並存。
- 選項 A：拆 main.tf 常駐段（貫徹「臨時起」架構，兌現安全顧慮）→ 動正式 IaC，下次 rebuild 生效
- 選項 B：保留常駐、wan-probe 直接打 5201（07-03 已實測 5201 可達）→ 撤回 ephemeral 敘述
- 未解：我不知道 5201 常駐是否還有其他消費者（如手動 debug 習慣）。**未驗證**。

### R2. Path C（P0-1）修 or 廢
run-round-only 鏈從未能在現部署模式下跑通（rsync 缺檔）。若 Path C 已被 run-vm6-suite 正式路徑取代，正解可能是**刪除**而非修復（三個 warmup + 三個 roundrun + orchestrator ≈ 100 行）。
- 未解：Path C 是否還有回歸價值（06-26 為 BLOCKER 2/3 而建）。需使用者確認再選修/刪。
...
prepare/run-all 32 檔鏡像、log 函式不共用、tests/common 一切、main.tf 5201（待 R1 拍板）、Makefile.tc1/tunnel.sh 刪除。
