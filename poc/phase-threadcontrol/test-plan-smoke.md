...
|---|---|
| Cell 數 | **1** |
| Topology | `vm-3node-3s3r-haproxy`（既有 baseline，符合 phase-threadcontrol/manifest.yaml allowed_topology）|
| DB | TiDB |
| Isolation | `rc` |
| Knob category | TiKV `readpool.unified.max-thread-count` |
...
其他參數**完全不動**（保留 vm-3node-3s3r-haproxy baseline 對應）。

## 3. 環境前置 / 假設

| 項 | 狀態 |
|---|---|
| 3 VM (.32/.33/.34) | **保留**目前 vm-3node-3s3r-haproxy 部署狀態（不 destroy） |
...
# dump TiKV readpool relevant settings (3 nodes)
for ip in 172.24.40.32 172.24.40.33 172.24.40.34; do
  ssh root@$ip 'tikv-ctl --host 127.0.0.1:20160 modify-tikv-config -n readpool.unified.max-thread-count' \
...
# === Stage 3: 採 after-config dump（驗證 patch 成功）===
for ip in 172.24.40.32 172.24.40.33 172.24.40.34; do
  ssh root@$ip 'tikv-ctl --host 127.0.0.1:20160 modify-tikv-config -n readpool.unified.max-thread-count' \
    > "$OUT/db-config/after-readpool-${ip##*.}.txt"
done

# === Stage 4: 跑 v4.7 suite ===
...
| # | 檔 | 內容 |
|---|---|---|
| 1 | `phase-threadcontrol/playbooks/apply-tidb-readpool.yml` | ansible playbook：set TiKV `readpool.unified.max-thread-count` + `auto-adjust-pool-size=false` + reload tikv-config |
| 2 | `phase-threadcontrol/playbooks/revert-tidb-readpool.yml` | revert above |
| 3 | （可選）`tests/common/lib/common.sh::write_phase_done` 加 phase/result_scope/baseline_eligible/tuning_profile_id metadata（codex v3 add D；本 smoke 先驗 path，metadata 留下輪 commit）|
...
| 日期 | commit | 變更 |
|---|---|---|
| 2026-06-06 | （本 commit）| 初版 smoke plan，Q1-Q6 拍板（cell=1 / topology=3s3r-haproxy / wrapper=thin / codex review enabled） |
