#!/usr/bin/env python3
"""Plot Sysbench Scenario #4 (TiProxy) results.

Style: QPS/TPS grouped bar chart; 95p latency line on twin axis.

Data (IDC *3 + GCP *3 同時執行 TiProxy perspective):
Workload               p95(ms)   QPS       TPS
oltp_read_only         81.48     3222.09   201.38
oltp_read_write        108.68    2534.71   126.74
oltp_write_only        41.85     1583.14   263.86
select_random_points   20.00     560.65    560.65
select_random_ranges   15.55     763.36    763.36

Usage:
  python sysbench_results_#4_tiproxy.py [--show] [--svg]
Outputs:
  sysbench_results_#4_tiproxy_summary.png (+ optional SVG)
"""
...
DATA: List[Row] = [
    Row("oltp_read_only", 81.48, 3222.09, 201.38),
    Row("oltp_read_write", 108.68, 2534.71, 126.74),
    Row("oltp_write_only", 41.85, 1583.14, 263.86),
    Row("select_random_points", 20.00, 560.65, 560.65),
    Row("select_random_ranges", 15.55, 763.36, 763.36),
]

PNG_NAME = "sysbench_results_#4_tiproxy_summary.png"
SVG_NAME = "sysbench_results_#4_tiproxy_summary.svg"
...
def table() -> str:
    ...
    for r in DATA:
        ...
        rows.append([r.name, f"{r.p95_ms:.2f}", f"{r.qps:.2f}", f"{r.tps:.2f}", f"{ratio:.2f}"])
...
def plot():  # pragma: no cover
    ...
    ax_thr.set_title('Sysbench Scenario #4 (TiProxy) - QPS/TPS Bars & 95p Latency Line')
...
def main():
    ap = argparse.ArgumentParser(description='Plot sysbench scenario #4 (TiProxy) QPS/TPS bars + latency line.')
