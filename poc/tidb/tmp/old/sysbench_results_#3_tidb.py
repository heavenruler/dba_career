#!/usr/bin/env python3
"""Plot Sysbench Scenario #3 (TiDB) results.

Style requirement: QPS/TPS use grouped bar charts; 95p latency uses a line on a twin y-axis.

Scenario #3 numbers (IDC *3 同時執行 TiDB perspective):
Workload               p95(ms)   QPS       TPS
oltp_read_only         86.00     2029.10   126.82
oltp_read_write        110.66    1881.49   94.07
oltp_write_only        26.20     2288.44   381.41
select_random_points   7.84      1835.68   1835.68
select_random_ranges   7.04      1853.29   1853.29

Usage:
    python sysbench_results_#3_tidb.py            # generate PNG
    python sysbench_results_#3_tidb.py --show     # print table then generate
    python sysbench_results_#3_tidb.py --svg      # also export SVG
"""
...
DATA: List[Row] = [
    Row("oltp_read_only", 86.00, 2029.10, 126.82),
    Row("oltp_read_write", 110.66, 1881.49, 94.07),
    Row("oltp_write_only", 26.20, 2288.44, 381.41),
    Row("select_random_points", 7.84, 1835.68, 1835.68),
    Row("select_random_ranges", 7.04, 1853.29, 1853.29),
]

PNG_NAME = "sysbench_results_#3_tidb_summary.png"
SVG_NAME = "sysbench_results_#3_tidb_summary.svg"
...
def plot():  # pragma: no cover
    ...
    # X axis labels (wrap underscores)
    ax_thr.set_xticks(x)
    ...
    ax_thr.set_title('Sysbench Scenario #3 (TiDB) - QPS/TPS Bars & 95p Latency Line')
...
def main():
    ap = argparse.ArgumentParser(description='Plot sysbench scenario #3 (TiDB) QPS/TPS bars + latency line.')
