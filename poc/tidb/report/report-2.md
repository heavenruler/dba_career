...
## **效能現象**
- 4 → 8 vCPU 均呈現 **明顯正成長（+15～25%）**
- SQL Layer 併行度提升 → RPC / Goroutine 排程更順暢

## **結論（Read-heavy）**
**Scale-Up 對 Read-heavy 完全有效；TiDB 的 SQL 層對 CPU 擴張高度敏感。**
...
## **效能現象**
- 4 → 8 vCPU 提升幅度 **約 +20～35%**
- SQL 層 CPU 增加 → 更有效推動 Prewrite/Commit

## **原因**
- 寫入瓶頸位於 TiKV（RocksDB + Raft）  
- SQL 層 CPU 擴張能提高「送出交易」的速率  
- TiKV 後段仍是主要限制，因此提升屬「有限度增長」

## **結論（Write-heavy）**
**TiDB 寫入在 Scale-Up 有明顯提升，但受 KV 端限制，成長幅度有限。**
...
## **效能現象**
- Mixed 同時壓 SQL（查詢）與 KV（寫入）  
- 8 vCPU 在 SQL 層的排程效率提升 → 帶動整體 QPS

## **結論（Mixed）**
**TiDB Mixed 類型對 CPU 擴張特別敏感；8vCPU 提升最為明顯。**
...
| 工作負載 | 最佳配置 | 原因 |
|-----------|------------|---------|
| Read-heavy | **多 SQL 單 KV** | 完全受 SQL 層併行能力主導 |
| Write-heavy | **多 SQL 單 KV** | SQL 層送出交易效率更高 |
| Mixed | **多 SQL 單 KV（小幅領先）** | 但瓶頸偏向 KV，差距縮小 |
| 整體最佳 | **多 SQL 單 KV** | SQL 層是最容易 Scale-Out 的部分 |
...
| 類型 | 單 SQL 多 KV → 多 SQL 單 KV | 差異 |
|------|-------------------------------|--------|
| read_only | 783.54 → 830.14 TPS | **+6%** |
| points | 5097.07 → 5538.02 TPS | **+8.6%** |
| ranges | 5598.35 → 7026.91 TPS | **+25.5%** |
...
| 類型 | 單 SQL 多 KV → 多 SQL 單 KV | 差異 |
|------|-------------------------------|--------|
| update_index | 3371.89 → 3946.03 TPS | **+17.1%** |
| write_only | 1500.30 → 1782.03 TPS | **+18.8%** |
...
| 類型 | 單 SQL 多 KV → 多 SQL 單 KV | 差異 |
|------|-------------------------------|--------|
| read_write | 503.92 → 561.45 TPS | **+11.4%** |
