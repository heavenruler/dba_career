...
* **高延遲 (RTT)：** 主要衝擊**前端應用層**（TiProxy/TiDB）的高併發處理能力 [12, 13]。在 RTT 200 ms 條件下，TiProxy 的 QPS 衰退高達 **93.73%** [13]，TiDB 也衰退 **86.58%** [13]。
* **低頻寬 (Bandwidth)：** 主要衝擊**後端儲存層的寫入**操作（TiKV Write）[12, 14]。當頻寬限制到 5 Mbps 時，TiKV Write 的 QPS 暴跌 **89.65%** [14, 15]，使其成為高併發低頻寬下的最大瓶頸 [14]。
* **讀取穩定性：** TiKV Read 在所有 RTT 測試中表現穩定 ，且在 5 Mbps 頻寬限制下，效能下降僅 **2.88%** 。
