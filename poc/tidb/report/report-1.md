...
- 低～中併發（10/50/100）：+0.8% / +1.6% / +2.8%，提升有限。
  - 原因：`SELECT 1` 幾乎無計算量，瓶頸在網路/連線處理/代理開銷；4vCPU 已足夠，擴到 8vCPU 不具線性效益。
- 中高併發（250/500）：+18.3% / +16.4%，出現明顯增益。
  - 原因：開始觸及 TiDB/TiProxy/網卡中斷分散極限，多核心可分攤系統呼叫/中斷/排隊成本。
- 超高併發（250/500/1000）：從 threads 100 後的 rps 明顯開始衰減。
  - 熱點影響：高併發下排隊/重傳/連線競爭放大，導致抖動與回落；將出現 leader/region 層的集中排隊，單點成為瓶頸，整體 RPS 下滑。

#### Scale 策略的影響

- 垂直擴展（Scale Up：4vCPU → 8vCPU）
  - 低～中併發（10/50/100）：提升有限（約 +1%～+3%）。SELECT 1 為無計算量查詢，瓶頸多在網路/連線處理/代理開銷，CPU 核數不是主導因素。
  - 中高併發（250/500）：出現可見增益（約 +16%～+18%）。多核心可分攤系統呼叫/排隊成本，改善吞吐。
  - 超高併發（1000）：RPS 數據衰減，屬壓測效能邊界。

- 水平擴展（Scale Out：TiDB/TiProxy/TiKV 節點增加）
  - SELECT 1 屬無計算量查詢
    - TiProxy / TiDB 前端一般可近線性擴展；但需確保連線分佈均衡（TiProxy/LB）、避免少數節點或 Region 形成熱點。
    - 增加 TiKV 節點無效益。

#### SELECT 1 RPS 概覽
- SELECT 1 的 RPS 擴展優先順序：先做前端水平擴展（TiDB/TiProxy），再針對 250～500 並發的場景做垂直擴展；同時調整系統與網路參數，避免 Threads 1000 的效能衰減。
