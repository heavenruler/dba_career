...
| threads | RPS(單點 IDC) | RPS(單區 IDC 4vCPU) | RPS(單區 IDC 8vCPU) | 差異%(8vCPU對4vCPU) |
| ------- | -------------- | ------------------- | ------------------- | ------------------- |
| 10      | 15395.67       | 96774.19            | 97560.98            | +0.8%               |
| 50      | 23433.84       | 95026.92            | 96587.25            | +1.6%               |
| 100     | 22627.85       | 91547.15            | 94132.41            | +2.8%               |
...
- 低～中併發（10/50/100）：8vCPU 對 4vCPU 僅小幅提升（+0.8%/+1.6%/+2.8%）。原因在於 SELECT 1 幾乎無計算量，瓶頸主要在網路、連線處理與代理開銷，CPU 不是主導因素。
- 中高併發（250/500）：出現明顯增益（約 +18.3%/+16.4%），反映多核心能分攤系統呼叫/中斷/排隊成本，吞吐改善有感。
- 超高併發（1000）：RPS 開始衰減，屬壓測邊界行為；常見成因為排隊/重傳/連線競爭以及請求熱點導致的 region/leader 集中排隊。

#### Scale 策略的影響

- 垂直擴展（Scale Up：4vCPU → 8vCPU）
  - 適合針對 250～500 併發的吞吐衝刺；對 10/50/100 提升有限。
  - 若要挑戰 1000 併發，需最佳化相關設定、Loac Balance / Connection Pool，避免效能衰減 ; 或透過水平擴展攤平流量熱點負載。

- 水平擴展（Scale Out：TiDB / TiProxy 節點數量）
  - SELECT 1 為前端無計算量查詢，擴充 TiDB/TiProxy 並確保連線均衡，通常可近線性提升；需避免少數節點或 Region 形成熱點。
  - 增加 TiKV 節點對 SELECT 1 無顯著效益。

#### SELECT 1 RPS 概覽
- 優先順序：先做前端水平擴展（TiDB/TiProxy + 均衡），再針對 250～500 並發做垂直擴展；1000 併發屬壓測邊界，僅供壓力容限參考。
- 風險控管：在進入 250～500 並發前完成系統/網路參數校正，並監控熱點避免局部排隊造成整體 RPS 下滑。
...
### 數據對照表（4 vCPU TiProxy/TiDB 單節點 vs 多節點：mysqlslap SELECT 1）

- 表頭說明
  - 組態 A（4vCPU #1）：TiProxy 1 節點、TiDB 1 節點、TiKV 3 節點（172.24.40.18/19/20）
  - 組態 B（4vCPU #2）：TiProxy 3 節點、TiDB 3 節點、TiKV 1 節點（172.24.40.20）
  - 比較口徑：同一 concurrency（threads）下，以 avg_qps 當作 RPS，計算 B 對 A 的差異%。

- 來源
  - TiDB(4vCPU #1)：TiDB + TiProxy @ IDC Cluster with 4 vCPU #1 @ mysqlslap_logs_20251027_092815
  - TiDB(4vCPU #2)：TiDB + TiProxy @ IDC Cluster with 4 vCPU #2 @ mysqlslap_logs_20251027_102800

- 欄位
  - threads | RPS(組態A：4vCPU #1) | RPS(組態B：4vCPU #2) | 差異%(B 對 A)

| threads | RPS(4vCPU #1) | RPS(4vCPU #2) | 差異%(B 對 A) |
| ------- | ------------- | ------------- | -------------- |
...
- 快速解讀
  - 10/50 併發近似；前端節點增加帶來極小幅度提升。
  - 100 併發時 B 大幅下降（-47.5%），推測與 TiKV 僅 1 節點導致後端服務能力不足或排隊放大有關（雖 SELECT 1 為前端查詢，仍受後端路徑/控制面排程影響）。
  - 250/500/1000 下 A 全面優於 B，顯示維持足量 TiKV 與穩定前端配置更能支撐高併發 RPS 穩態。

-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------

### 數據對照表（8 vCPU TiProxy/TiDB 單節點 vs 多節點：mysqlslap SELECT 1）

- 表頭說明
  - 組態 C（8vCPU #1）：TiProxy 1 節點、TiDB 1 節點、TiKV 3 節點（172.24.40.18/19/20）
  - 組態 D（8vCPU #2）：TiProxy 3 節點、TiDB 3 節點、TiKV 1 節點（172.24.40.20）
  - 比較口徑：同一 concurrency（threads）下，以 avg_qps 當作 RPS，計算 D 對 C 的差異%。

- 來源
  - TiDB(8vCPU #1)：TiDB + TiProxy @ IDC Cluster with 8 vCPU #1 @ mysqlslap_logs_20251027_155357
  - TiDB(8vCPU #2)：TiDB + TiProxy @ IDC Cluster with 8 vCPU #2 @ mysqlslap_logs_20251027_154712

- 欄位
  - threads | RPS(組態C：8vCPU #1) | RPS(組態D：8vCPU #2) | 差異%(D 對 C)

| threads | RPS(8vCPU #1) | RPS(8vCPU #2) | 差異%(D 對 C) |
| ------- | ------------- | ------------- | -------------- |
...
- 快速解讀
  - 10 併發無差異；50～250 併發下，C 明顯優於 D（TiKV 3 節點帶來較穩定吞吐）。
  - 1000 併發時 D 高於 C（+18.7%），可能因多個前端節點在極高併發下更能攤平連線/排隊開銷；此點屬邊界行為，需搭配系統與網路調校評估。

### 數據對照表（avg[s]：TiDB & MySQL，IDC 叢集 8 vCPU）

- 表頭說明
  - 比較對象：IDC 叢集 8 vCPU 的 MySQL(+ProxySQL) vs TiDB(+TiProxy) 在 SELECT 1 的 avg(s)（整批完成平均耗時，RTT+排隊近似）。
  - 比較口徑：同一 threads，比較 avg(s) 越低越好；差異% = (TiDB/MySQL - 1)×100。
...
- 欄位
  - threads | MySQL avg(s) | TiDB avg(s) | 差異%(TiDB 對 MySQL)
...
| 250     | 1.44          | 2.13         | +47.9%               |
| 500     | 4.03          | 8.43         | +109.2%              |
| 1000    | 9.39          | 12.86        | +36.9%               |

- 快速解讀
  - 低併發（10/50）：TiDB avg(s) 較低（更快），與 RPS 結果一致。
  - 100 併發：TiDB avg(s) 略高（+5%），對應 RPS 中 MySQL 略優。
  - 高併發（≥250）：TiDB avg(s) 明顯上升（較慢），與 RPS 落後一致；需倚賴前端水平擴展與系統參數調優。

### 數據對照表（avg[s]：4 vCPU TiDB 單/多節點對照，IDC 叢集）

- 表頭說明
  - 組態 A（4vCPU #1）：TiProxy 1 節點、TiDB 1 節點、TiKV 3 節點。
  - 組態 B（4vCPU #2）：TiProxy 3 節點、TiDB 3 節點、TiKV 1 節點。
  - 比較口徑：同一 threads 比較 avg(s)；差異% = (B/A - 1)×100。
...
- 欄位
  - threads | avg(s)(A) | avg(s)(B) | 差異%(B 對 A)
...
| 250     | 2.52       | 2.78       | +10.3%         |
| 500     | 9.81       | 10.46      | +6.6%          |
| 1000    | 12.04      | 12.99      | +7.9%          |

- 快速解讀
  - 10/50：差異極小。
  - 100：B 大幅較慢（+90.8%），與其 RPS 大幅下滑一致；TiKV 單節點造成後端/排程瓶頸。
  - 250/500/1000：B 一致較慢，反映高併發下前端/後端資源配置與排隊差異。

### 數據對照表（avg[s]：8 vCPU TiDB 單/多節點對照，IDC 叢集）

- 表頭說明
  - 組態 C（8vCPU #1）：TiProxy 1 節點、TiDB 1 節點、TiKV 3 節點。
  - 組態 D（8vCPU #2）：TiProxy 3 節點、TiDB 3 節點、TiKV 1 節點。
  - 比較口徑：同一 threads 比較 avg(s)；差異% = (D/C - 1)×100。
...
- 欄位
  - threads | avg(s)(C) | avg(s)(D) | 差異%(D 對 C)
...
| 250     | 2.13       | 2.78       | +30.5%         |
| 500     | 8.43       | 9.44       | +12.0%         |
| 1000    | 12.86      | 10.84      | -15.7%         |

- 快速解讀
  - 50～250：D 較慢 30% 左右，對應其 RPS 落後。
  - 1000：D 較快（-15.7%），對應 RPS 邊界點 D > C 的現象；需合併系統/網路調校與負載特性來評估是否具代表性。
