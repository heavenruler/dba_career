...
- 表頭說明：
  ...
  - 兩組皆為 IDC 單區 4 vCPU 叢集，對比「前端（TiProxy/TiDB）節點數」與「TiKV 節點數」差異對 SELECT 1 RPS 的影響。
- 來源：
...
- 欄位：threads | RPS(組態A：4vCPU #1) | RPS(組態B：4vCPU #2) | 差異%(B 對 A)
...
- 快速解讀：
  - 10/50 併發下兩組近似，前端節點增加帶來極小幅度提升。
  - 100 併發時，組態 B 大幅下降（-47.5%），推測為 TiKV 僅 1 節點導致後端服務能力不足或排隊放大（雖 SELECT 1 主要為前端查詢，仍受後端路徑/控制面與排程影響）。
  - 250/500/1000 併發下，組態 A 全面優於組態 B，顯示 TiKV 足量與穩定前端配置更能維持高併發下的 RPS 穩態。
...
- 表頭說明：
  ...
  - 兩組皆為 IDC 單區 8 vCPU 叢集，對比前端節點數與 TiKV 節點數差異對 SELECT 1 RPS 的影響。
- 來源：
...
- 欄位：threads | RPS(組態C：8vCPU #1) | RPS(組態D：8vCPU #2) | 差異%(D 對 C)
...
- 快速解讀：
  - 10 併發無差異；50～250 併發下，單節點前端 + 3 節點 TiKV（#1）明顯優於多節點前端 + 1 節點 TiKV（#2）。
  - 1000 併發時 #2 高於 #1（+18.7%），可能因多 TiDB/TiProxy 前端在極高併發下更能吸收連線/排隊開銷；但該點屬壓測邊界，需搭配系統與網路參數最佳化評估。
