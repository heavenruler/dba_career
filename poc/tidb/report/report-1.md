...
| 1000    | 7773.63  | 9223.67  | +18.7%         |
 ...
 - 快速解讀
  ...
  - 1000：#2 反超（+18.7%），多前端能攤平連線/排隊；但 TiProxy/TiDB 新增連線/轉發固定成本對短查詢明顯，需靠 連線複用 / Connection Pool 與網路/IRQ 調優才能穩定放大效益（邊界行為）。
  - 多一跳代理與轉發路徑：相比 MySQL 走 ProxySQL 一層，TiDB 常見路徑是「Client → TiProxy → TiDB」多一層中繼。每次請求要多經過一次轉送與檢查，就像多轉一次手、再做一次簡單的記錄與包裝，雖然每次只多一點點時間，但在 SELECT 1 這種超短操作、而且併發很高的情況下，這些「一點點」會被放大，疊起來就會吃掉可見的 RPS。

### 數據對照表（三組 TiDB：單機4 / 叢集4#2 / 叢集8#2，mysqlslap SELECT 1）

- 表頭說明
  - 組態 A（單機 4vCPU）：TiDB + TiProxy 單機；tidb component 在單機
  - 組態 B（叢集 4vCPU #2）：TiDB + TiProxy 叢集；tidb component 分散在各 VM
  - 組態 C（叢集 8vCPU #2）：TiDB + TiProxy 叢集；tidb component 分散在各 VM；scale-up（8 vCPU）
  - 比較口徑：同一 threads，以 avg_qps 當 RPS；差異%(B 對 A)=(B/A-1)×100、差異%(C 對 B)=(C/B-1)×100。

- 來源
  - A：TiDB + TiProxy @ Single Instance with 4 vCPU @ mysqlslap_logs_20251021_133658
  - B：TiDB + TiProxy @ IDC Cluster with 4 vCPU #2 @ mysqlslap_logs_20251027_102800
  - C：TiDB + TiProxy @ IDC Cluster with 8 vCPU #2 @ mysqlslap_logs_20251027_154712

- 欄位
  - threads | RPS(A 單機4) | RPS(B 叢集4) | RPS(C 叢集8) | 差異%(B 對 A) | 差異%(C 對 B)

| threads | RPS(A 單機4) | RPS(B 叢集4) | RPS(C 叢集8) | 差異%(B 對 A) | 差異%(C 對 B) |
| ------- | ------------- | ------------- | ------------- | -------------- | -------------- |
| 10      | 15395.67      | 97339.39      | 97560.98      | +532.7%        | +0.2%          |
| 50      | 23433.84      | 96556.16      | 72797.86      | +312.0%        | -24.6%         |
| 100     | 22627.85      | 48076.92      | 72236.94      | +112.5%        | +50.2%         |
| 250     | 21792.82      | 35979.85      | 35928.14      | +65.1%         | -0.1%          |
| 500     | 20781.38      | 9555.66       | 10588.73      | -54.0%         | +10.8%         |
| 1000    | 19677.29      | 7699.22       | 9223.67       | -60.8%         | +19.8%         |

- 快速解讀
  - 單機 → 叢集（4 vCPU）：10/50/100/250 下，叢集 RPS 大幅高於單機（+65%～+533%），顯示分散在多 VM 的單組前端能大幅提升短查詢吞吐；500/1000 時叢集反而低於單機，推測為前端/後端資源與排隊配置在極高併發下未對齊（池化/連線上限/IRQ/網路）。
  - scale-up（叢集 4 vCPU → 8 vCPU）：50 明顯下降（-24.6%），100/500/1000 有感提升（+50.2% / +10.8% / +19.8%），250 幾乎持平。說明 8 vCPU 的效益主要在更高併發（≥100）開始顯現；低中併發可能因排程/池化參數未同步調整而未現增益。
  - 操作建議：針對 50～250 併發的主力區間，叢集化顯著優於單機；8 vCPU 的增益在 100+ 併發更明顯。500～1000 併發屬邊界，需同步調整 TiProxy/TiDB 連線池、後端連線上限、RSS/RPS/irqbalance 與 MTU，並考慮前端水平擴展攤平排隊。

### 數據對照表（三組 TiDB：單機4 / 叢集4#1 / 叢集8#1，mysqlslap SELECT 1）

- 表頭說明
  - 組態 A（單機 4vCPU）：TiDB + TiProxy 單機；tidb component 在單機
  - 組態 B（叢集 4vCPU #1）：TiDB + TiProxy 叢集；tidb component 分散在各 VM；scale-out
  - 組態 C（叢集 8vCPU #1）：TiDB + TiProxy 叢集；tidb component 分散在各 VM；scale-up（8 vCPU）
  - 比較口徑：同一 threads，以 avg_qps 當 RPS；差異%(B 對 A)=(B/A-1)×100、差異%(C 對 B)=(C/B-1)×100。

- 來源
  - A：TiDB + TiProxy @ Single Instance with 4 vCPU @ mysqlslap_logs_20251021_133658
  - B：TiDB + TiProxy @ IDC Cluster with 4 vCPU #1 @ mysqlslap_logs_20251027_092815
  - C：TiDB + TiProxy @ IDC Cluster with 8 vCPU #1 @ mysqlslap_logs_20251027_155357

- 欄位
  - threads | RPS(A 單機4) | RPS(B 叢集4#1) | RPS(C 叢集8#1) | 差異%(B 對 A) | 差異%(C 對 B)

| threads | RPS(A 單機4) | RPS(B 叢集4#1) | RPS(C 叢集8#1) | 差異%(B 對 A) | 差異%(C 對 B) |
| ------- | ------------- | -------------- | -------------- | -------------- | -------------- |
| 10      | 15395.67      | 96774.19       | 97560.98       | +528.8%        | +0.8%          |
| 50      | 23433.84      | 95026.92       | 96587.25       | +305.6%        | +1.6%          |
| 100     | 22627.85      | 91547.15       | 94132.41       | +304.5%        | +2.8%          |
| 250     | 21792.82      | 39719.32       | 46977.76       | +82.3%         | +18.3%         |
| 500     | 20781.38      | 10192.99       | 11862.40       | -50.9%         | +16.4%         |
| 1000    | 19677.29      | 8306.57        | 7773.63        | -57.8%         | -6.4%          |

- 快速解讀
  - 單機 → 叢集（4 vCPU）：10/50/100/250 下，叢集 RPS 遠高於單機（+82%～+529%）；500/1000 時叢集低於單機，顯示極高併發下需同步連線池/後端上限/IRQ/網路調校。
  - scale-up（叢集 4vCPU → 8vCPU）：50/100/250/500 有感提升（+1.6% / +2.8% / +18.3% / +16.4%）；1000 輕微回落（-6.4%），為邊界行為。
