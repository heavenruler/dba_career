...
| 段落 | 這一段回答什麼 |
|---|---|
| **北極星** | 目標沒變，但實務終點被 Y26 的驗證結果修正到哪裡 |
| **Y25 結論** | 驗了什麼、明確不驗什麼、留下哪些既成事實 |
| **Y26 結論** | 四家候選怎麼定位、能做什麼、不能做什麼 |
| **推進路線** | 技術主線、產品期待、維運後勤三條軌怎麼並行 |
| **實施辦法** | 採用 TiDB 的 Phase 0–3 與各階段驗收 |

> 全篇數字皆可回溯至 `poc/results/`、`poc/phase-crossregion/` 的原始測試證據。
