...
S-BASE TPC-C 對標測試已完成：TiDB、CockroachDB、YugabyteDB 均完成 VM、K8s-unlimit、K8s-limit 變體；正式結果彙整於 [`poc/results/README.md`](../results/README.md)，各 DB 詳細紀錄位於 `poc/results/<db>-tc1/S-BASE/pipeline-log.md`。

下一步：依測試結果整理管理層摘要與顧問討論材料，並視需要將 K8s YAML / Ansible vars 納入 IaC 正式交付範圍。
