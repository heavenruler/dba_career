...
| **決策** | 可執行選項、ROI 判準、Pilot Gate 與本次需要確認的事項 |
...
| 執行效率 | TiDB 已有 S-BASE、Kubernetes、跨區與故障測試鏈，可直接重用 Gate、結果與自動化 |
| TSD 回饋 | 目前重視一致性、服務恢復速度、故障範圍隔離及 Database Self-Service，未要求直接推進跨區 A/A |

**本階段輸出**：應用相容性、PITR／還原、Online DDL、服務恢復、資源隔離、原廠支援與 TCO／ROI 的可驗收結果。
...
> Infra 改造必須同時說明產品效益、應用改造量與 ROI；技術可行不等於推進理由。
...
| **B｜快速驗證＋Pilot** | **TiDB 完成技術 Gate，再導入一個代表性 MySQL 應用／批次** | **最快取得應用、維運、Self-Service 與 ROI 證據** | 需 TSD owner、應用樣本與回復窗口 |
...
| G4｜商務可行 | 原廠支援、授權、維運人力、基礎設施與風險降低 | TCO／ROI／go-no-go 建議 |
...
`三年 ROI =（三年量化收益 − 三年增量成本）÷ 三年增量成本`

**放行條件**：技術硬 Gate 通過、假設有 owner 與範圍、敏感度分析完成，且 ROI／回收期達公司門檻。現階段價格仍為 TBD，不宣稱已證明正 ROI。
...
- 官方文件星等不計入已測項目加權分數；真正導入結論以應用相容性、營運演練、原廠條款與 TCO 為準。
