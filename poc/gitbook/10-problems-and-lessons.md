...
**最後驗證日期：2026-07-21**（首次驗證 2026-07-15）
...
- [本 PoC 實測｜N=1] **同一個 bash 陷阱在不同腳本重演，且都在「fail-closed 驗收正確運作」下才被抓到。** `while read ... do ssh ...; done <<< "$VAR"` 這個結構——迴圈內若 `ssh`（或任何會讀 stdin 的命令）未把自己的 stdin 導向 `/dev/null`，會偷走迴圈自身的 here-string 輸入，導致迴圈只跑第一輪就提前結束。此陷阱先在 `ybdb-master-quorum-gate.sh`（07-08 節）出現過一次，**07-20 在新寫的 `merge-gcp-stdout.sh` 又重演**——20 筆待合併檔案只處理了 1 筆，且兩次都是靠下游 fail-closed 驗收（分別是 gate 自身重試邏輯、`check-aaro-artifacts.py`）攔下才發現，未讓半套結果檔案靜默通過。教訓：這類迴圈寫法本身有已知風險，不能只靠事後補驗收，新腳本若用到「迴圈內呼叫會讀 stdin 的外部命令」應預先加 `< /dev/null`，而非等它在生產環境重演第三次。
