...
| 2 | 單節點/低併發延遲 | 延遲與水平擴展 | 19% | go-tpc TPC-C，`vm-1node` RC，W=128（見 §3.2.1） | ⭐⭐⭐⭐⭐ | ⭐☆☆☆☆ |
| 3 | 水平擴展能力 | 延遲與水平擴展 | 24% | go-tpc TPC-C，`vm-1node`→`vm-3node-haproxy-3s3r` 擴展比（見 §3.2.2） | ⭐☆☆☆☆ | ⭐⭐⭐⭐⭐ |
| 4 | 高併發穩定性 | 延遲與水平擴展 | 19% | go-tpc TPC-C，t=128 5-round range/mean 與 error rate（見 §3.2.3） | ⭐☆☆☆☆ | ⭐⭐⭐⭐⭐ |
| 5 | Failover RTO／RPO | Failover、RTO／RPO、PITR | 5% | 已實測，但 Galera G2 與 TiDB F2 的情境語意及計時起點不等價，暫不換算星等（見 §3.3.1a） | 待重新評估 | 待重新評估 |
...
| 8 | HTAP／TiFlash | TiDB HTAP／TiFlash | 5% | 分析型查詢尚未排入（見 §3.5） | n/a | 待測 |
| | **合計** | | **100%**† | | | |

† 原始配置合計 88%，缺少的 12% 平均補入項目 2～4，各增加 4%。**目前可計分項目為
#2/#3/#4（同拓樸 `vm-1node`/`vm-3node-haproxy-3s3r` 實測），合計 62% 權重；#5
已有 chaos/failover 原始數據，但因情境與計時起點不等價而暫不計分；#1/#6/#7/#8
尚未計分**（見 §4.1 加權總分與其限制）。PXC/Galera 的 #8 為 `n/a`，不代表其具備
尚未驗證的 HTAP 能力。
...
> `results/{tidb,crdb,yuga,galera}-tc1/S-BASE/pipeline-log.md`。口徑：W=128、
...
> 驗證，不构成決策級結論。Galera 的 `vm-1node`/`vm-3node-haproxy-3s3r` 數字為
> 2026-08-13 實測（TS=`20260813T073744+0800`／`20260813T112044+0800`），
> canonical artifact 見
...
|---|---:|---:|---:|---:|
| MySQL Galera Cluster | t=32 | 53,791.9 | 37.7 ms | 0.000% |
...
**MySQL 相容群組星等（2026-08-13 補測；僅 Galera vs TiDB 內部比較）**：Galera
37.7ms → ⭐⭐⭐⭐⭐；TiDB 597ms（約 15.8 倍） → ⭐☆☆☆☆（見 §2.1）。**Fact**：即使改用
Galera 自己 t=128 的數字（51,527.8 tpmC，p99=182.8ms，見 §3.2.3）仍遠優於 TiDB
t=128 的 597ms，不是代表點 thread 數不同造成的錯覺。**Inference（非 fact）**：這
很可能反映的是「vm-1node 對 TiDB 而言仍是 PD+TiKV+TiDB-server 三元件在同一台 VM
上協調運作」的架構開銷（即使只有一個邏輯節點，元件間仍有 RPC/Raft 協調成本），
而 Galera 在單節點模式下 wsrep 幾乎是空轉（無其他成員可複寫），效果上接近原生
單機 MySQL/InnoDB——**這不能推論成「Galera 整體優於 TiDB」**，只能說「在單節點
這一個特定情境下，原生 MySQL 引擎的延遲優勢明顯」，且此優勢在 §3.2.2/§3.2.3
的多節點情境下完全反轉（見下方）。
...
|---|---:|---:|---:|---:|
| MySQL Galera Cluster | 53,791.9 | 26,166.2 | **0.49×** | 未測（Galera 無 direct-3s3r 對照組，見下方說明） |
...
Galera 0.49×（**負向**，加節點後吞吐量反而降到單節點的 49%） → ⭐☆☆☆☆。**Fact**：
`vm-3node-haproxy-3s3r` 對 Galera 是 3 台完整副本（不是 3 shard×RF3，見
[`ansible/playbooks/galera-vm3.yml`](./ansible/playbooks/galera-vm3.yml) 開頭
說明），HAProxy 以 round-robin 把寫入分散到全部 3 台（multi-writer），這與
TiDB/CRDB/YBDB 的「多節點分攤負載」擴展機制不是同一種——Galera 每多一個
併發寫入節點，就多一份跟其他節點 certification 衝突的機會。**Fact（wsrep
counter，post-run 單點 snapshot，非嚴格 before/after delta）**：`.32` 節點
`wsrep_local_cert_failures=234`、`wsrep_local_bf_aborts=265`（相對
`wsrep_local_commits=1,559,041`，約佔 0.015%~0.017%）、
`wsrep_flow_control_paused=22.4%`。**Inference**：certification 衝突本身佔比
不算高，但 0.49× 的巨大衰退更可能主要來自 flow control 暫停與跨節點寫入排序
的同步成本，certification 衝突只是其中一部分貢獻——**具體佔比未經本次證據
拆解**，不應斷言 certification 衝突是唯一或主要根因。**不可下的結論**：不能
因為 Galera 這裡是負向擴展，就推論「Galera 完全不能多寫」——本測試刻意用
HAProxy round-robin 製造最大化的多寫衝突情境，若應用層改用單寫（見 §3.6 的
P-A×A-S 設計）或以 shard key 分流降低熱點衝突，結果可能截然不同，本節數字
只反映「無任何應用層優化下的 naive multi-writer」情境。

#### 3.2.3 高併發穩定性（t=128，5-round range/mean 與 error rate）

| DB | tpmC (t=128 mean) | range/mean（5 輪間變異） | error rate |
|---|---:|---:|---:|
| MySQL Galera Cluster | 26,166.2 | 43.2%（t=16 最高達 117.5%，見下方） | 0.037%（all_txn） |
...
在 t=128 分別是 21,627.0／19,886.8／27,146.5／30,977.8／31,192.8——不是穩定
在某個水位小幅震盪，而是有明顯的低點與高點分佈；t=16 更極端（10,960.8／
11,911.4／5,421.5／2,860.4／13,314.8，range/mean 117.5%，最低與最高輪相差
近 4.7 倍）。error rate 隨 thread 數上升（0.012%→0.037%），與 TiDB/YBDB/CRDB
全部維持 0.000% 形成對比——這是四家中唯一觀測到非零 all-txn 錯誤率的一家。
**Inference**：型態與 §3.2.2 討論的 multi-writer certification/flow-control
成本一致，但沒有逐輪的 wsrep counter delta（本次只有 post-run 單點
snapshot），無法證實「哪一輪特別低是因為哪一次衝突高峰」，僅記錄現象。
...
- **星等換算**（依 §2 分組規則，僅在 PostgreSQL 相容群組內比較；依 F2 真實復原時間為
  ...
  **TiDB 為 MySQL 相容群組成員，此段落僅評 PostgreSQL 相容群組內部星等；Galera 已於
  ...
  不代表 TiDB 的容錯能力「較差」。
- **MySQL Galera Cluster（2026-08-13 已補測，G1-G5，見下方獨立小節）**：Galera 的多主複寫
  模型與本 PoC 既有的 raft-based 三家測試方法論不同，量測方式另外設計（G1-G5，不是
  F1/C4/F2/C1/C7 的別名），完整數字與評分狀態見 §3.3.1a。
- **本項星等的已知限制**（務必與星等一併閱讀）：
  ...
  - 探測皆從 IDC 側（`.31`）發起，未驗證 GCP 側 client 的真實體驗（C1 情境尤其明顯，見比較
    報告 §4/§10）。
  - F2 是「停止 3 台 IDC DB process，再由 operator 立即重啟」的恢復力測試，不是自動區域
    Failover。2026-08-11 的 CockroachDB 重跑使用 W=4，YugabyteDB 重跑使用 W=128；雖然
    兩家各自與 2026-08-08 的方向一致，工作集大小仍非完全對等，因此 #5 星等只作群組內
    方向性判讀，不應轉成 SLA 或精確倍率。
...
#### 3.3.1a MySQL 相容群組：Galera（PXC 8.4）chaos/failover 實測（2026-08-13）

> **設計基礎**：Galera 沒有 leader/follower，F1（leader kill）/C4（leader-follower
> 對照）整套機制（graceful resign 指令、leader 查詢）對 Galera 不適用，改設計 5 個新
> 情境 G1-G5（見
> [`phase-crossregion/GALERA-EXECUTION-PLAN.md`](./phase-crossregion/GALERA-EXECUTION-PLAN.md)
> Stage 5 可重用度分類表）：G1（單節點 kill，IDC/GCP 區域軸取代 leader/follower 軸，
> 骨架沿用 F1/C4）、G2（殺全部 3 台 IDC，機制沿用 F2 但改測 quorum-loss 而非區域
> failover）、G3（雙寫衝突風暴，全新設計）、G4（跨區網路分斷，沿用 C1 機制）、G5
...
- **G2（殺全部 3 台 IDC，quorum-loss）— Fact**：`cluster_rebuild_sec=22.169`、
  ...
  write-reject 驗證出現 `UNEXPECTED_WRITE_SUCCEEDED_review_manually`——kill 後
  約 14 秒時對 GCP 端發出的 INSERT/DELETE 實際成功（mysql 靜默無輸出，非靜默失敗），
  最可能的原因是存活的 3 台 GCP 尚未偵測到對端消失（`evs.suspect_timeout`/
  `evs.inactive_timeout` 尚未到期）便短暫繼續接受寫入，之後才降級為 non-Primary
  （inference，未做逐秒 wsrep 狀態輪詢以precise 定位偵測延遲的確切秒數）。
  **關鍵語意差異（務必與 TiDB 的 F2 一併理解，不可只比較秒數）**：兩個情境都由
  operator 重啟 IDC 節點，均未驗證 GCP 自動接手。TiDB 的 39.1～44.3 秒是
  `t_restart_start` 到 `t_first_write_ok`；若從停止 IDC 節點的 `t_kill` 起算，則為
  198.4～201.7 秒。Galera 的 22.169 秒是 `t_kill` 到 `t_first_write_ok`，其中
  `t_restart_start` 到首次寫入約 8.240 秒。**兩者的故障注入、恢復流程及計時起點
  不等價，因此不能用這些秒數判定產品 Failover 優劣。**Galera 在本次 6-node
  單一叢集設計下，GCP 端 3 台無法獨立形成 Primary Component（majority=4）；這是
  本次部署可確認的架構限制，但不能反向推論 TiDB F2 已證明 GCP 自動接手。
- **G1（單節點 crash kill，GCP 側）— Fact**：`rto_sec=null`（`outage_observed=false`，
  432/432 探測皆成功）、`rpo_lost_tx_count=0`——殺 1/6 節點對 client 端寫入完全無
  可觀測影響（5/6 仍 ≥ majority=4），與 TiDB/CRDB/YBDB 的 F1/C4 單節點 kill 結果
  方向一致（三家皆觀測不到或觀測到極短中斷）。
- **G4（跨區網路 3v3 全斷）— Fact**：主腳本的 SELECT-1 粗探測全程顯示 `ok`（僅證明
  mysqld 存活，`SELECT 1` 不經過 InnoDB/wsrep，無法驗證寫入可用性）；額外用真實寫入
  補測（非主腳本一部分，見 fetch-receipt.json 的 `known_limitation` 說明）：IDC 側
  收到 `Error 1213 Deadlock`、GCP 側收到 `Error 1047 WSREP has not yet prepared
  node for application use`——**兩側皆正確拒絕寫入**（3v3 兩邊都低於 majority=4），
  與 `RTO-RPO-methodology.md` §2.1 對 P-B row 的既有預期（「兩區皆 minority ⇒ 全
  cluster 寫拒」）一致。分斷解除後約 20~30 秒內自動經 IST/SST 恢復 6/6 Primary，
  未需人工介入。
- **G3（雙寫衝突風暴，P-B×A-A）— Fact**：30 秒集中衝突負載下，6 節點 wsrep counter
  的 workload 前後 delta（本次首次做到逐次 delta，不是先前 §3.2/§3.6 的單點
  snapshot）：`wsrep_local_cert_failures` 合計 474、`wsrep_local_bf_aborts` 合計
  313，相對 `wsrep_local_commits` delta 1849（約 25.6%／16.9%）。但兩端注入腳本
  自己回報 `err=0`（完全零客戶端可見失敗）——**Fact**：`wsrep_retry_autocommit=1`
  （已用 `SHOW VARIABLES` 確認，PXC 預設值）。**Inference**：這很可能解釋了兩者的
  落差——單一 autocommit 陳述式失敗會被 PXC 自動重試一次而不回報給 client，但
  go-tpc 的 PAYMENT 交易是多陳述式的顯式 transaction，不符合這個自動重試的資格，
  這可能是 §3.6.2 真實 TPC-C 跑出遠高於本次的 client 端錯誤率的原因之一（未經
  額外對照實驗證實，僅為方向性解釋）。
- **G5（disk-slow）**：fio 干擾成功啟動（`fio_launch_ok=true`，需先在全新 VM 上
  安裝 fio），延遲數字需查 `io-latency-p99.txt`/`fio-summary.txt`，比照 TiDB/CRDB/
  YBDB 的 C7 慣例不產出通過/失敗判定。
- **MySQL 相容群組第 5 項狀態：待重新評估。**Galera G2 與 TiDB F2 的原始記錄仍保留，
  但兩者都包含 operator 重啟，且計時起點不同，不具備換算同一組星等的比較基礎。
  若要恢復計分，需先定義相同的故障邊界、禁止人工恢復的觀測窗、相同探測位置，以及
  `t_kill` 到首次成功寫入的統一 RTO 口徑；RPO 也須由相同交易序列驗證。Galera 的
  quorum 設計限制可作架構判讀，但不能用來替代等價的 Failover 實測。
...
> **狀態說明**：本節是 2026-08-12 新增的實測數據，**與 §2.1 加權評分項目 #2/#3/#4
...
|---|---|---:|---:|
| MySQL Galera Cluster | #2 #3 #4 | 62% | (5×19 + 1×24 + 1×19) / 62 × 20 = **44.5** |
| TiDB | #2 #3 #4 | 62% | (1×19 + 5×24 + 5×19) / 62 × 20 = **75.5** |

> 計算方式同 §4.2 範例。**本群組尚未計分的項目為 #1（相容性 20%）、#5
> （Failover 5%）、#6（PITR 3%）、#7（Online DDL 5%）及 #8（HTAP 5%），合計
> 38% 權重**。目前的 44.5／75.5 只反映 #2～#4 的延遲、水平擴展與高併發穩定性
> （62% 權重），**不是整體評分**，更不能簡化成「TiDB 整體優於 Galera 31 分」。
> PXC/Galera 的 #8 為 `n/a`，其實際適用權重為 95%。#5 已有原始數據，但因比較
> 口徑不等價而暫停換算星等，不等同於「未執行」。
>
> 這個差距的成因也不是單一方向：#2（單節點延遲）Galera 大幅領先（源自 §3.2.1
> 討論的「TiDB vm-1node 仍有 PD/TiKV/TiDB-server 協調開銷」推論），但 #3/#4（水平
> 擴展/高併發穩定性）TiDB 大幅領先，方向與 §3.2.2/§3.2.3 討論的 Galera
> multi-writer certification/flow-control 成本一致。**選型時應該分別看這幾個
> 子分數，而非只看合計的 44.5/75.5**——若應用
> 情境是單機/低併發（如小型部署、報表庫），Galera 這裡的數字反而更有利；若需要
> 水平擴展或高併發穩定性，TiDB 的已測數字較有利。區域級 Failover 則須待 #5 以
> 等價情境重測後再判斷。
...
| DB | 已計入項目 | 已計入權重合計 | 加權總分（百分制） |
|---|---|---:|---:|
| YugabyteDB | #2 #3 #4 #5 | 82% | (5×24 + 4×29 + 4×24 + 5×5) / 82 × 20 = **87.1** |
| CockroachDB | #2 #3 #4 #5 | 82% | (3×24 + 5×29 + 5×24 + 4×5) / 82 × 20 = **87.1** |
...
**現況**：本群組**已產出部分加權總分**（見 §4.1）——§2.1 的 #2/#3/#4（同拓樸延遲/
擴展/穩定性）合計 62% 權重，Galera 44.5 分／TiDB 75.5 分。#5 chaos/failover 已完成
原始測試，但因 Galera G2 與 TiDB F2 的情境及計時起點不等價，暫列「待重新評估」且
不計分；#1/#6/#7/#8 仍待測。未計分項目合計 38% 權重，**這不是完整加權評分**。另外
2026-08-12 完成的跨區（`vm-6node`）P-A/P-B
穩態吞吐量實測（見 §3.6）是**另一個不同的測試維度**（同機房 vs 跨區），不計入
§2.1/§4.1 的星等，但同樣提供有價值的方向性參考。

**§3.3.1a chaos/failover 發現**（G1-G5，fact/inference 分層見 §3.3.1a 原文，此處
僅摘要方向）：Galera G2 與 TiDB F2 都由 operator 重啟 IDC 節點，均未驗證 GCP
自動接手。Galera 的 22.169 秒採 `t_kill` 到首次寫入，TiDB 的 39.1～44.3 秒採
`t_restart_start` 到首次寫入；因此不能直接排名。Galera 在本次 6-node 單叢集下，
GCP 3 節點不足 majority=4，仍是明確的架構限制，但不能以此替代等價 Failover 實測。
單節點 kill（G1）與跨區
分斷（G4，含補測的真實寫入驗證）兩家型態一致（正確拒絕/無感知影響），無方向性
差異；G3 補上了先前一直缺的 wsrep counter workload 前後 delta 證據（cert_failures
delta=474／bf_aborts delta=313，見 §3.3.1a），是本次額外的方法論貢獻，不是
DB 間比較。

**§3.2 同拓樸發現**（vm-1node／vm-3node-haproxy-3s3r，fact/inference 分層見
§3.2.1/§3.2.2/§3.2.3 原文，此處僅摘要方向）：單節點情境 Galera 大幅領先 TiDB
（NEW_ORDER p99 37.7ms vs 597ms，fact），推論可能來自 TiDB 即使在單 VM 部署下
仍保有 PD/TiKV/TiDB-server 三元件協調開銷，而 Galera 單節點下 wsrep 幾乎空轉
（inference）。但這個優勢在加了 2 台節點、HAProxy round-robin 分散寫入後**完全
反轉**：Galera 的 tpmC 從單節點 53,791.9 掉到 3 節點 haproxy 情境的 26,166.2
（**0.49×，負向擴展**，fact），TiDB 則從 13,064 成長到 26,947（2.06×，正向擴展，
fact）；Galera 同時觀測到明顯更差的 5-round 穩定度（range/mean 43.2% vs TiDB
7.4%，fact）與四家中唯一非零的 all-txn error rate（0.037%，fact）。這個反轉方向
上與「Galera multi-writer 需要跨節點 certification，TiDB 的多節點是分攤負載而非
互相驗證」的架構差異一致（inference），且有 wsrep counter 佐證衝突確實發生
（`wsrep_local_cert_failures=234`／`wsrep_local_bf_aborts=265`，fact），但
counter 只是 post-run 單點 snapshot，無法拆解「衝突成本」與「flow control 同步
成本」各佔多少（inference 的界線，見 §3.2.2 原文）。
...
悲觀鎖競爭有關，未證實）；Failover 方面，F2 從 operator 開始重啟到首次寫入為
39.1～44.3 秒，從停止 IDC 節點到首次寫入則為 198.4～201.7 秒，F1/C4 單節點 kill 有 6.68~8.4s 的真實
可觀測中斷，write-reject 判定乾淨（`PD server timeout`）。

**不能下的結論**：不能因為 §4.1 的 44.5/75.5 或 §3.6 的跨區吞吐量/失敗率差距就
直接斷言「TiDB 整體優於 Galera」——§2.1 尚未計分的 #1/#5/#6/#7/#8 合計 38% 權重；
已計分結果只涵蓋延遲、水平擴展及高併發穩定性。也不能只看 44.5/75.5 合計分就忽略
#2 是 Galera 大幅領先、#3/#4 是 TiDB 大幅領先；選型時應分別評估，不是相加抵銷。
**#5 目前不得用星等或總分解讀**：原始記錄可用於改善下一輪實驗設計，但不足以形成
產品 Failover 排名。§3.6 的跨區數字額外有 P-B
缺獨立 collect 證據、缺 wsrep counter delta 的限制（見 §3.6 的 lineage caveat
與 inference 標註）；不能把 Galera 的 P-A/P-B（client routing profile）與 TiDB
的 P-A/P-B（server-side placement）當作語意等價的 A/B test 直接換算擴展率；也
不能把 TiDB 的 Failover 秒數與 YugabyteDB／CockroachDB 的「無可觀測中斷」做倍率
換算（見 §3.3.1，兩者不是同一種量測基準）。

### 5.2 PostgreSQL 相容群組：YugabyteDB vs CockroachDB

**現況**：兩家皆已完成 §3.2 效能測試與 §3.3.1 Failover 真實重跑（2026-08-11），
加權總分 YugabyteDB（87.1）／CockroachDB（87.1）（見 §4.2）——**兩者在目前已計分的
82% 權重範圍內同分，不宜下「何者較優」的結論**。
...
1. **（2026-08-13 已完成）** MySQL Galera Cluster 已補測與本 PoC §3.2 相同口徑
   ...
   MySQL 相容群組目前可產出 #2/#3/#4（合計 62%）的星等與 §4.1 部分加權總分
   （Galera 44.5／TiDB 75.5）。#5 已有原始數據，但比較口徑不等價，暫列「待重新
   評估」；連同 #1 相容性、#6 PITR、#7 Online DDL、#8 HTAP，未計分權重合計 38%。
