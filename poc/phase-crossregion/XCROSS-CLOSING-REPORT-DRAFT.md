...
| **TiDB#1**（首輪） | ❌ 備查（t128 range% 102.2%，§7） | [tidb-vm-6node-P-A-rc-20260711T215200+0800](../results/x-cross/baseline/w128/20260711T215200+0800/tidb-vm-6node-P-A-rc-20260711T215200+0800/) |
...
- **三家同批同鏈**：單一 [`make win-3db-detach`](Makefile) driver 於 .31 依序跑 TiDB→YBDB→CRDB，每 cell static-check 全綠才進下一家；三 suite `manifest_sha256` 相同、時序連續——交接間隔 7m38s／1m50s（由前家 `.suite.done` 完成時戳 → 下一家 `prepare/go-tpc-prepare.log` 首行時戳回推）。「全程零人工介入」為操作紀錄（消 §8 O3/O9，證據等級見 §8.1 O9）。
...
| OS / kernel | AlmaLinux 8.10，4.18.0-553.124.1.el8_10（DB hosts）；**OS 調校限制**：THP 未關、TiDB 主機 ulimit -n=1024（見 §8 O10） | §9.1 env 列 |
...
口徑：tpmC_mean 為 R1-R5 mean；變異一律以 **range%**（= `tpmC_range_mean_pct` = (max−min)/mean）呈現——
**range/mean 口徑，非樣本標準差 CV**，對單一離群輪敏感（如 YBDB t64：range% 81.1%
vs 樣本 CV 35.1%）；歷史文件曾將此口徑稱作「CV」，本報告已統一改稱 range% 以免與統計 CV 混淆。延遲 = NEW_ORDER
p50/p95/p99 mean (ms)。錯誤率 = `error_count / (total_count + error_count)`，
分母為**嘗試總數（成功＋錯誤）**——`tests/common/summary-from-stdout.py` 的
`rate()` 實作口徑；`total_count` 本身只計成功數，本報告引用時並列成功／嘗試
兩個分母以便追溯。逐輪原始值在各 `summary.json` 的
`thread_results.<t>.tpmC_per_round`；逐輪完整輸出在各 round 的 `go-tpc-stdout.txt`（§9.1）。

### 5.1 吞吐

| threads | TiDB tpmC (range%) | CRDB tpmC (range%) | YBDB tpmC (range%) |
|---:|---:|---:|---:|
...
|---|---|---|---|---|---|
| TiDB | 12,526.5 tpmC、p99 677.8ms | t128 range% 5.7%，五輪緊密 | 0（成功 1,384,230） | X-CROSS 內 TiDB P-A/A-S 有效參考值 | t16 前二輪偏低未歸因（§6.1）；與 07-03/07-12 批的跨批變異未歸因（§8 O6） |
| CockroachDB | 10,163.4 tpmC、p99 1,020.1ms | t128 range% 19.2%（R4=8,673 單輪下降） | 0（成功 2,294,569） | X-CROSS 內 CRDB P-A/A-S 有效參考值 | t128 單輪下降未歸因（§6.2）；t64→t128 負縮放（§6.4） |
| YugabyteDB | 12,769.5 tpmC、p99 758.3ms | t128 range% 13.0%；t64 含一次深塌（R4=3,893） | 156（成功 2,167,333、嘗試 2,167,489；總率 ≈0.0072%；各檔 0.001-0.012%） | 可引用，**必須附 §6.3 caveat** | 殘餘 timeout 錯誤未歸零（§8 O1）；S1 修復為 prepare 時一次性 |

## 6. 各資料庫觀察與限制

### 6.1 TiDB

- `實測事實`：t16 五輪 = 1121.8 / 1082.1 / 1894.1 / 1905.1 / 1918.8——前二輪明顯偏低（約 -43%），R3 起穩定，造成 range% 52.8%。原始值：TiDB#3 [summary.json](../results/x-cross/baseline/w128/20260717T143238+0800/tidb-vm-6node-P-A-rc-20260717T143238+0800/summary.json) `thread_results.16.tpmC_per_round`；[t16 R1 原始輸出](../results/x-cross/baseline/w128/20260717T143238+0800/tidb-vm-6node-P-A-rc-20260717T143238+0800/runs/threads-16/round-1/go-tpc-stdout.txt)。
...
- `實測事實`（組態）：TiDB 的 HAProxy backend 為 `172.24.40.32:4000, 172.24.40.33:4000, 10.160.152.11:4000`、`balance roundrobin`（ansible playbook／haproxy.cfg.j2 定義）。`機制推論`：roundrobin 下約 1/3 連線的 SQL 層落在 GCP tidb-server（比例為組態推論，無 HAProxy session 統計佐證），每個 statement 跨 WAN 回 IDC 的 TiKV/PD。
...
- `實測事實`：t128 五輪 = 10562.0 / 10628.7 / 10526.0 / **8673.0** / 10427.5——R4 單輪下降約 -17%，造成 t128 range% 19.2%。原始值：CRDB#3 [summary.json](../results/x-cross/baseline/w128/20260717T143238+0800/crdb-vm-6node-P-A-rc-20260717T143238+0800/summary.json) `thread_results.128.tpmC_per_round`；[t128 R4 原始輸出](../results/x-cross/baseline/w128/20260717T143238+0800/crdb-vm-6node-P-A-rc-20260717T143238+0800/runs/threads-128/round-4/go-tpc-stdout.txt)。
- `根因未確認`：R4 下降未歸因（0 error、無 retry 訊息）；孤立單輪、未連續。前批（CRDB#2）t16 的 15.2% 波動在本批未重現（t16 range% 3.8%）。
...
- **實測事實**：t16 / t32 / t64 / t128 分別出現 3 / 9 / 59 / 85 筆交易錯誤（率 0.001-0.012%；成功 2,167,333、嘗試 2,167,489，總率 156/2,167,489 ≈ 0.0072%）；#2 批為 309 筆（0.0149%）——跨批觀察，非 S1 效果之證明。錯誤樣本可於各 round 檔 grep `execute run failed`（§9.1 raw go-tpc 列）。
...
- `觀察`：t64 R4=3892.8 深塌（其餘四輪 10698-12166，t64 range% 81.1%）——59 筆錯誤中 43 筆在 R4、16 筆在吞吐正常的 R2；R2 有錯誤而無塌陷反證「錯誤量→塌陷」的直接因果，且失敗交易占比（<0.25%）在量級上不足以解釋 -66% 降幅，兩者更可能是同一上游短暫不穩的並發症狀（`機制推論`，未定案）。
...
- **重跑結果**（實測事實）：TiDB#2 同參數重跑，t128 五輪 = 13188.2 / 13087.6 / 13268.1 / 13099.1 / 13614.8，range% 4.0%；異常未重現。修正後快照顯示 tpcc leader 只在 3 台 IDC store（8/8/3）、0 GCP：TiDB#2 [leader-snapshot/tidb-region-leaders.txt](../results/x-cross/baseline/w128/20260712T164221+0800/tidb-vm-6node-P-A-rc-20260712T164221+0800/leader-snapshot/tidb-region-leaders.txt)。
...
- **後續佐證**（實測事實）：TiDB#3（07-17）t128 五輪 12222.2-12933.5、range% 5.7%，異常連續第二批未重現。
...
| ID | 問題 | 修正 | 驗證證據 | 殘留限制 |
|---|---|---|---|---|
| C1 | CRDB#1/YBDB#1 的 GCP 節點零 tpcc 資料副本（07-13 覆核發現） | 五項：CRDB constraints counted-form；YBDB live 2+1 + 統一 zone；probe 直打 GCP 節點；probe 主機補裝 DB clients（`.15` 原無 client，四 suite probe 全滅實為 command-not-found）；CRDB gate grep 整行計數 bug（經授權修 `prepare.sh`）。防再犯：`gcp-replica-gate.sh` fail-closed、static-check probe 斷言、`phase2-probe-clients` | CRDB#2/YBDB#2 起三重驗證（placement／GCP 副本／probe）通過；#3 批三家同樣通過（OS soft-gate 除外，見 **O10**）（§3 連結；§9.1 矩陣） | YBDB 系統層 tablet 另立 **O1**（S1 gate 已上線，錯誤未歸零） |
...
| O9 | 流程穩定性尚未經「零人工介入」完整輪驗證（CRDB#2 過程含三修） | #3 全鏈連續通過：TiDB→YBDB→CRDB 每 cell deploy→smoke→static-check→teardown→歸檔，含全部 **fail-closed** gate（placement／gcp-replica／probe／static-check；OS gate 為記錄不阻擋之 soft-gate，本批記錄 fail，見 **O10**）；suite 級起訖 14:32→01:40:02（≈11h07m）為 artifact 事實 | artifact：三 suite `.suite.done`/`summary.json` 時戳＋交接間隔（`.suite.done`→下一家 prepare log 首行時戳；無長時間停頓之旁證）。「零人工介入」本身為**操作紀錄**（[SESSION-HISTORY.md](SESSION-HISTORY.md) 2026-07-17 節；鏈級 driver marker 在 .31 未入庫，marker 亦不記錄人工操作有無） | 單次通過（N=1）；缺輪斷言/歸檔等新 gate 為本輪首驗 |
...
| O10 | OS soft-gate 未過且先前未揭露（07-18 audit 補）：`實測事實`——#3 三家與 O1 驗證輪的 `.gate.done` 均 `os_gate_pass:false`；`os-gate.txt` 載 THP=`[always]`（未關閉）、TiDB 主機 `ulimit -n=1024`。`gate.sh` 設計為記錄不阻擋，pipeline 照跑 | OS 調校非 DB 最佳實務；**三家同受影響、批間一致**（同一 VM template），跨家相對比較的有效性影響有限，但絕對值不可外推至調校後環境 | 不阻擋（同批一致偏置，須揭露） | VM template 關 THP＋調 ulimit 後重跑（屆時與現值構成調校前後對照）；或維持現狀永久標注 |
...
`觀察`：各檔位較 #3 批低約 10-30%（t16 −17.7%、t32 −30.0%、t64 −9.8%、
t128 −13.7%；t64 差距最小，部分因 #3 批 t64 均值被 R4 深塌壓低）但變異更緊
——跨批變動的又一數據點（§8 O6 口徑，未歸因）。
