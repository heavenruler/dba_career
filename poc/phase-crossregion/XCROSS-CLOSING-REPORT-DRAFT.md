...
1. `採用決策`：正式採用三個 cell——**TiDB#4、CRDB#2、YBDB#2**（各 DB 一個 W=128、P-A、A-S cell；定義見 §3）。
...
5. `根因未確認`：TiDB#1 首輪 t128 下降（§7）、TiDB#4 t32 單輪下降（§6.1）、YBDB 錯誤的完整因果鏈（§6.3）——三者均無法以現有證據定案。
...
| 縮寫 | 狀態 | Suite 目錄 |
|---|---|---|
| **TiDB#4**（重跑） | ✅ 採用 | [tidb-vm-6node-P-A-rc-20260712T164221+0800](../results/x-cross/baseline/w128/20260712T164221+0800/tidb-vm-6node-P-A-rc-20260712T164221+0800/) |
...
| 執行入口 | TiDB#4：Mac 端 TiDB-only 鏈（07-12）；CRDB#2/YBDB#2：[`make win-ybdb-crdb-detach`](Makefile) `.31` detached 指揮鏈（07-14） | [SESSION-HISTORY.md](SESSION-HISTORY.md) 2026-07-12 / 07-14（續）節 |
...
- **重跑結果**（實測事實）：TiDB#4 同參數重跑，t128 五輪 = 13188.2 / 13087.6 / 13268.1 / 13099.1 / 13614.8，CV 4.0%；異常未重現。修正後快照顯示 tpcc leader 只在 3 台 IDC store（8/8/3）、0 GCP：TiDB#4 [leader-snapshot/tidb-region-leaders.txt](../results/x-cross/baseline/w128/20260712T164221+0800/tidb-vm-6node-P-A-rc-20260712T164221+0800/leader-snapshot/tidb-region-leaders.txt)。
- **根因未確認**：首輪下降的根因無法確認——首輪 VM 已銷毀，無 TiKV metrics 可回溯。
- **採用決策**：因高變異且同參數不可重現，TiDB#1 不納入正式結果、保留備查；採用 TiDB#4。詳細過程：[SESSION-HISTORY.md](SESSION-HISTORY.md) 2026-07-11/12 兩節；commits `621f24f1`、`5c4a9bcc`。
...
| O2 | TiDB#1 首輪下降、TiDB#4 t32 單輪下降均根因未確認 | 不影響採用 cell 的有效性判定 | 不阻擋 | 時間允許時以 N=3 重跑檢查重現性 |
| O3 | 採用三 cell 橫跨兩批：TiDB#4（07-12）vs CRDB#2/YBDB#2（07-14 同批）；且 TiDB#4 量測時 CRDB/YBDB 語意修正尚未存在 | 嚴格同批同語意的三家數據不存在；跨家並讀須註明批次 | 不阻擋條件式引用 | 若結案要求同批：以現行管線（含 gcp-replica-gate + probe 斷言）重跑三家 suite（~11 hr） |
