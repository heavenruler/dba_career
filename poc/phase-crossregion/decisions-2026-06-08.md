...
## Q16: Wave 4 六視窗全矩陣範圍（2026-07-07 拍板）

**Source**: Win-2 前必解缺口 #3（session_prompt / fable-refactor 健檢）；六視窗矩陣是對早期「P-A × A-S × 3-DB」隱性 3-cell 範圍的正式擴張。

**Decision**: Wave 4 steady-state 範圍 = **{P-A, P-B} × {A-S, A-A-RO, A-A} × {TiDB, CRDB, YBDB} = 6 視窗 × 3 家 = 18 cells**，依 Win-1…Win-6 順序推進。

### 規則
1. 每 cell 皆 per-cell full VM rebuild（承 Q11）；CV 口徑 R1–R5 mean（承 Q15）；W=128 N=5 threads∈{16,32,64,128}（承 Q6）
2. **baseline_eligibility 分層**：
   - **A-S**（Win-1/Win-4）= X-CROSS 內主數據候選（仍受 X-CROSS `baseline_eligible=false` 對外排名限制，Q11 澄清不變）
   - **A-A-RO / A-A**（Win-2/3/5/6）= **exploratory-only observed envelope**，只作機制/情境觀察，不入任何對照口徑
3. profile 間切換 = 新 cell（同 Q11：換 profile 與換 placement/DB 一視同仁，都重建）
4. A-A / A-A-RO 需雙 client（IDC + GCP g-test-poc-5）orchestration，前置 chrony drift <100ms（承 Q6/Q10）；GCP client 走 .31 直連（不走 IAP）——進 Win-2 前須 live 驗 FW 涵蓋 .31→GCP client SSH
5. 捨棄方案：只跑 P-A 三 profile（放棄 P-B 對照）——被使用者六視窗指示否決；一次跑滿 18 cell 不分視窗——排程/驗收粒度過大，維持逐視窗 fail-closed 收尾

## Q17: Artifact 目錄命名納入 PROFILE token（2026-07-07 拍板）

**Source**: Win-2 前必解缺口 #2；現行命名 `{db}-vm-6node-{P-A|P-B}-rc-{ts}` 不含 workload profile → 同 placement 不同 profile（A-S/A-A/A-A-RO）會撞名、混淆 static-check / promotion glob / summary。

**Decision**: profile token 插在 **placement 與 iso(`rc`) 之間**，且 **A-S 為 canonical token-less 預設**，A-A/A-A-RO 帶明確 token：

| profile | 目錄名 | 範例 |
|---|---|---|
| A-S | `{db}-vm-6node-{P}-rc-{ts}`（**不變**） | `tidb-vm-6node-P-A-rc-20260703T092243+0800` |
| A-A-RO | `{db}-vm-6node-{P}-aaro-rc-{ts}` | `crdb-vm-6node-P-A-aaro-rc-<ts>` |
| A-A | `{db}-vm-6node-{P}-aa-rc-{ts}` | `ybdb-vm-6node-P-B-aa-rc-<ts>` |

### 規則與理由
1. **A-S 不帶 token**：既有已採用 cell（`tidb-vm-6node-P-A-rc-20260703T092243`，pipeline-log §2.3）不改名（禁改 artifact 名），Win-1 三家 A-S 亦沿用；只有 A-A/A-A-RO 是需標記的變體。
2. **token 小寫 `aa`/`aaro`**（非 literal `A-A`）：避免 `P-A-A-A` 對人與 glob 的歧義；`*-P-A-rc-*` 只中 A-S、`*-P-A-aa-rc-*` 只中 A-A、`*-P-A*-rc-*` 中全 P-A。
3. **token 藏進 topology 段** → 三個 `tests/common/` 不可改檔**零改動**：`lib/common.sh:artifact_dir()`（吃 topo 參數）、`run.sh` 的 `vm-6node-*` 前綴檢查、`summary-from-stdout.py` 的 `(.+?)` lazy match（profile 併入 topology 欄）皆自然相容；`check-static-artifacts.py` 的 `vm-6node-*` glob 亦不受影響。
4. **需同步修改（皆可改檔，僅為 A-A/A-A-RO 生效）**：`run-vm6-suite.sh:115`、`win-tidb-as-w128.sh:49`（+ crdb/ybdb 對應）、`Makefile` ~13 處 ROOT 拼接（引入 `PROFILE` make-var → `PROFILE_TOKEN`：A-S 空、A-A `-aa`、A-A-RO `-aaro`）、`promotion-gate.sh:45/66`（A-A/A-A-RO promotion 檢查另加，現行 #1/#2 針對 A-S/P-B 不動）。
5. **summary.json 影響**：A-A/A-A-RO cell 的 `topology` 欄會是 `vm-6node-P-A-aa`；需乾淨 placement 的 consumer 讀 `placement-gate-*.json` 的 `placement` 欄（summary.py 本就這樣取，不靠目錄名）。A-S cell topology 欄不變。
6. 捨棄方案：**全 profile 皆帶 token**（含 A-S `-a-s-`）——既有 A-S cell 需改名（禁），且 Win-1 三家會與已採用 TiDB cell 格式不一致；**改 `tests/common` regex 加 profile group**——踩不可改清單。

---

## 雙端（IDC + GCP client）數據彙整規則（2026-07-15 拍板，grilling 六題）

**背景**：既有 tpmC/p95/p99 以 IDC 端 client 為主；P-B / A-A-RO / A-A 情境將加入
GCP 端 go-tpc 紀錄，須先定義彙整口徑避免混側。

| # | 決定 | 內容 |
|---|---|---|
| G1 | **永不合併** | IDC 主表不動；GCP 端自成一表；p95/p99 任何情況不跨側混合（兩側延遲分布異質，混合會產生不存在的假分布） |
| G2 | **GCP RO 端吞吐欄 = `read-tpmTotal`** | A-A-RO 的 GCP mix 無 NEW_ORDER，tpmC 依定義恆 0（指標無定義，非量到零）；改取 go-tpc `[Summary]` 的 tpmTotal＋逐交易型（ORDER_STATUS/STOCK_LEVEL）延遲，欄名明標非 tpmC、不可與 IDC tpmC 比大小 |
| G3 | **同 suite 目錄雙子樹** | GCP 端輸出落 `runs/threads-N/round-M/go-tpc-stdout-gcp.txt`（與 IDC 檔並排同 round 目錄）；`summary.json` 新增 `gcp_side.thread_results`，IDC 既有欄位零變動 |
| G4 | **一 profile 一主表、一側一列、以側分區** | 上半段三家 idc 列（同側同量綱可跨家並讀）、下半段三家 gcp 列；長表 schema 三 profile 一致，不用寬表並欄 |
| G5 | **A-A 的 gcp 列照標 tpmC** | 雙端同 mix 時 gcp 端是真 tpmC，允許與 idc 列並讀對照（差距＝跨 WAN 寫入代價，A-A 核心產出）但不加總；表尾附 `cluster 合計 tpmC` 參考列（僅吞吐、不含延遲、不進主排序） |
| G6 | **P-B×A-S 保留為 placement 單因子對照** | 單 IDC client（GCP 端 probe 照跑、不跑 go-tpc）；對照鏈每步只動一變數：P-A×A-S → P-B×A-S（placement 效應）→ P-B×A-A-RO（+唯讀 client）→ P-B×A-A（+雙寫） |
