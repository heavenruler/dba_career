...
直連 TiDB 與透過 TiProxy 差異 #5-1_#5-2_compare.py
...
建議: 針對 TiDB 直連聚焦 200 threads 附近調優 (熱點/調度), TiProxy 維持多節點以支撐 100~250 / 250~750 波段。
```
...
IDC / GCP TiDB 叢集效能差異
```
IDC: TiProxy 對 TiDB 最大增益出現在 750 threads (+56.2%).
IDC: 退化點 threads = [1, 200] (低併發或特定峰值下 TiProxy 不利)。
GCP: TiProxy 全部 threads 均低於 TiDB，最大跌幅在 1 threads (-33.4%).
跨環境 TiDB GCP 相對 IDC：主要提升在 500 threads (+122.8%)，部分 threads 反而下降 (250 -25.7%).
跨環境 TiProxy GCP 相對 IDC 100/250/750 顯著下滑 (100 -53.2%, 250 -55.4%, 750 -55.6%).
結論: IDC 環境 TiProxy 可在多個中高併發點提供 >+50% 吞吐; GCP 中 TiProxy 尚未展現優勢，偏向優化連線/路由或觀察網路延遲因素。
```
![](./%235-1_%235-2_%236-1_%236-2_env_compare.png)
