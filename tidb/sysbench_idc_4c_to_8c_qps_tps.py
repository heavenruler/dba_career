...
def plot():  # pragma: no cover
    ...
    def bars(ax, metric: str):
        ...
        # Annotate baseline (4c) values
        for bars_ in (b1, b3):
            for bar in bars_:
                h = bar.get_height()
                ax.text(bar.get_x()+bar.get_width()/2, h*1.01, f"{h:.0f}", ha='center', va='bottom', fontsize=7)

        max_h = max([bar.get_height() for bars_ in (b1, b2, b3, b4) for bar in bars_])
        offset = max_h * 0.025

        # Annotate 8c raw + delta (red +, green -)
        for idx in range(len(WORKLOADS)):
            # TiDB
            h4 = b1[idx].get_height(); h8 = b2[idx].get_height()
            pct = (h8 / h4 - 1.0) * 100 if h4 else 0.0
            x_ti = b2[idx].get_x()+b2[idx].get_width()/2
            ax.text(x_ti, h8*1.01, f"{h8:.0f}", ha='center', va='bottom', fontsize=7)
            ax.text(x_ti, h8*1.01 + offset, f"{pct:+.1f}%", ha='center', va='bottom', fontsize=7,
