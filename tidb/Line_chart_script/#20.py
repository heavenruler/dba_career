    ...
    for line in table_text.splitlines():
        ...
        if len(parts) >= 10:
            try:
                ...
                rows.append({'oltype': oltype_short,'latency_95': latency_95,'qps': qps,'tps': tps})
            except ValueError:
                continue
    return rows

def plot_section(data_tidb, data_tiproxy, title):
...
def main():
    md_path = 'sysbench.md'
    section_header = 'IDC * 3 (8vCPU 16GB Ram)'
    ...
    plot_section(data_tidb, data_tiproxy, 'IDC*3 (8vCPU 16GB Ram): TiDB vs TiProxy Sysbench')
