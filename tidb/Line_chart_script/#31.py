...
MD_FILE = 'sysbench.md'
SCENARIO_HEADER = 'proxy.local-tidb-only enabled'
...
def plot_performance_grid(data: Dict[str, List[SysbenchResult]]):
    """Creates and shows the combined 2x2 plot."""
    fig, axes = plt.subplots(2, 2, figsize=(16, 12), sharex=True)
    
    _plot_performance(axes[0, 0], 'IDC Access (Off-Peak)', data['tidb_idc'], data['tiproxy_idc'])
    _plot_performance(axes[0, 1], 'GCP Access (Off-Peak)', data['tidb_gcp'], data['tiproxy_gcp'])
    _plot_performance(axes[1, 0], 'Simultaneous IDC Access (Off-Peak)', data['tidb_idc_both'], data['tiproxy_idc_both'])
    _plot_performance(axes[1, 1], 'Simultaneous GCP Access (Off-Peak)', data['tidb_gcp_both'], data['tiproxy_gcp_both'])
...
def main():
    ...
    headers = {
        'tidb_idc': 'Benchmark from TiDB with IDC # 離峰',
        'tiproxy_idc': 'Benchmark from TiProxy with IDC # 離峰',
        'tidb_gcp': 'Benchmark from TiDB with GCP # 離峰',
        'tiproxy_gcp': 'Benchmark from TiProxy with GCP # 離峰',
        'tidb_idc_both': 'Benchmark from TiDB with IDC both Execute # 離峰',
        'tiproxy_idc_both': 'Benchmark from TiProxy with IDC both Execute # 離峰',
        'tidb_gcp_both': 'Benchmark from TiDB with GCP both Execute # 離峰',
        'tiproxy_gcp_both': 'Benchmark from TiProxy with GCP both Execute # 離峰'
    }
    ...
    for key, header in headers.items():
        ...
        if not block:
            print(f"Warning: Could not find or extract data for '{header}'")
            parsed_data[key] = []
            continue
        parsed_data[key] = parse_table(block)

    if not any(parsed_data.values()):
        print("No data could be parsed. Aborting.")
        return

    plot_performance_grid(parsed_data)
