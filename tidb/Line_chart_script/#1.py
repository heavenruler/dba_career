...
# Extract the "- IDC * 1" section
idc1_section = re.search(r'- IDC \* 1(.*?)(={5,}|- GCP \* 1)', content, re.DOTALL)
if not idc1_section:
    raise ValueError("IDC * 1 section not found")
section_text = idc1_section.group(1)

# Table titles and colors
tables = [
    ("RPS From TiDB", "TiDB", "blue"),
    ("RPS From TiDB passthrough A10 NAT", "TiDB-NAT", "red"),
    ("RPS From TiProxy", "TiProxy", "green"),
    ("RPS From TiProxy passthrough A10 NAT", "TiProxy-NAT", "orange"),
]
...
ax1.set_title('IDC * 1 - RPS Line Chart')
ax1.grid(True)
ax1.legend(loc='upper left')

ax2.set_xlabel('Threads')
ax2.set_ylabel('Error Rate (%)')
ax2.set_title('IDC * 1 - Error Rate Line Chart')
ax2.grid(True)
