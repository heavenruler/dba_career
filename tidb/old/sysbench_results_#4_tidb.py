#!/usr/bin/env python3
"""Plot Sysbench Scenario #4 (TiDB) results.

Style: QPS/TPS grouped bar chart; 95p latency line on twin axis.

Data (IDC *3 + GCP *3 同時執行 TiDB perspective):
Workload               p95(ms)   QPS       TPS
oltp_read_only         81.48     3309.73   206.86
oltp_read_write        108.68    2565.35   128.27
oltp_write_only        42.61     1580.42   263.40
select_random_points   18.95     599.81    599.81
select_random_ranges   15.55     710.56    710.56

Usage:
  python sysbench_results_#4_tidb.py [--show] [--svg]
Outputs:
  sysbench_results_#4_tidb_summary.png (+ optional SVG)
"""
...
DATA: List[Row] = [
    Row("oltp_read_only", 81.48, 3309.73, 206.86),
    Row("oltp_read_write", 108.68, 2565.35, 128.27),
    Row("oltp_write_only", 42.61, 1580.42, 263.40),
    Row("select_random_points", 18.95, 599.81, 599.81),
    Row("select_random_ranges", 15.55, 710.56, 710.56),
]

PNG_NAME = "sysbench_results_#4_tidb_summary.png"
SVG_NAME = "sysbench_results_#4_tidb_summary.svg"
...
def plot():  # pragma: no cover
    ...
    ax_thr.set_title('Sysbench Scenario #4 (TiDB) - QPS/TPS Bars & 95p Latency Line')
...
def main():
    ap = argparse.ArgumentParser(description='Plot sysbench scenario #4 (TiDB) QPS/TPS bars + latency line.')
