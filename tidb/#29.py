...
def plot_performance_comparison(tidb_on_peak, tiproxy_on_peak, tidb_off_peak, tiproxy_off_peak):
    ...
    fig.suptitle(f'Performance Comparison for "{SCENARIO_HEADER}" (Access from GCP)', fontsize=16, y=0.98)
...
def main():
    ...
    # Extract data for each of the four conditions
    tidb_on_peak_block = extract_benchmark_data(scenario_content, 'Benchmark from TiDB with GCP # 上班時段')
    tiproxy_on_peak_block = extract_benchmark_data(scenario_content, 'Benchmark from TiProxy with GCP # 上班時段')
    tidb_off_peak_block = extract_benchmark_data(scenario_content, 'Benchmark from TiDB with GCP # 離峰時段')
    tiproxy_off_peak_block = extract_benchmark_data(scenario_content, 'Benchmark from TiProxy with GCP # 離峰時段')

    # Parse the extracted data blocks
    tidb_on_peak_data = parse_table(tidb_on_peak_block)
    tiproxy_on_peak_data = parse_table(tiproxy_on_peak_block)
    tidb_off_peak_data = parse_table(tidb_off_peak_block)
    tiproxy_off_peak_data = parse_table(tiproxy_off_peak_block)

    # Check if all data was found
    if not all([tidb_on_peak_data, tiproxy_on_peak_data, tidb_off_peak_data, tiproxy_off_peak_data]):
        print("Could not parse all required data blocks. Check headers in markdown file.")
        return

    # Plot the data
    plot_performance_comparison(
        tidb_on_peak_data, tiproxy_on_peak_data,
        tidb_off_peak_data, tiproxy_off_peak_data
    )
